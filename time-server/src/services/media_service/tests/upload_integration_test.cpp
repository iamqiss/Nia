...
using namespace time::media_service;
...
int run_upload_test() {
    ...
    auto storage = std::shared_ptr<StorageBackend>(CreateLocalStorage("/tmp/time-media-test", "file:///tmp/time-media-test").release());
    ...
    assert(server);
    fs::create_directories("/tmp/time-media-test");
    ...
    auto stub = std::make_unique<::time::media::MediaService::Stub>(channel);
    grpc::ClientContext ctx; ctx.AddMetadata("x-user-id", "u1");
    ::time::media::UploadResponse resp;
    auto writer = stub->Upload(&ctx, &resp);
    ::time::media::UploadRequest init;
    auto* i = init.mutable_init(); i->set_owner_user_id("u1"); i->set_type(::time::media::MEDIA_TYPE_IMAGE); i->set_mime_type("image/png");
    ...
    ::time::media::UploadRequest chunk; chunk.mutable_chunk()->set_content(std::string(buf.begin(), buf.end()));
