...
protected:
    void SetUp() override {
        ...
        e2e_manager_ = std::make_unique<time::messaging::crypto::E2EEncryptionManager>();
    }
    ...
    std::unique_ptr<time::messaging::crypto::E2EEncryptionManager> e2e_manager_;
};
...
TEST_F(SecurityFeaturesTest, MLSCreateGroup) {
    ...
    auto result = mls_protocol_->create_group(group_id, 
                                             time::mls::CipherSuite::MLS_128_DHKEMX25519_AES128GCM_SHA256_Ed25519,
                                             extensions);
...
TEST_F(SecurityFeaturesTest, MLSAddMember) {
    ...
    time::mls::KeyPackage key_package;
...
TEST_F(SecurityFeaturesTest, PQCKyberKeyGeneration) {
    auto keypair = pqc_algorithms_->generate_kyber_keypair(time::pqc::PQCAlgorithm::KYBER_768);
    
    EXPECT_FALSE(keypair.public_key.empty());
    EXPECT_FALSE(keypair.private_key.empty());
    EXPECT_EQ(keypair.algorithm, time::pqc::PQCAlgorithm::KYBER_768);
...
TEST_F(SecurityFeaturesTest, PQCKyberEncapsulation) {
    ...
    auto ciphertext = pqc_algorithms_->kyber_encapsulate(keypair.public_key, time::pqc::PQCAlgorithm::KYBER_768);
    EXPECT_FALSE(ciphertext.empty());
    EXPECT_EQ(ciphertext.size(), 1088);
    
    auto shared_secret = pqc_algorithms_->kyber_decapsulate(ciphertext, keypair.private_key, time::pqc::PQCAlgorithm::KYBER_768);
    EXPECT_FALSE(shared_secret.empty());
    EXPECT_EQ(shared_secret.size(), 32);
}

TEST_F(SecurityFeaturesTest, PQCDilithiumSignVerify) {
    auto keypair = pqc_algorithms_->generate_dilithium_keypair(time::pqc::PQCAlgorithm::DILITHIUM_3);
    
    std::string message = "Test message for Dilithium signature";
    std::vector<uint8_t> message_bytes(message.begin(), message.end());
    
    auto signature = pqc_algorithms_->dilithium_sign(message_bytes, keypair.private_key, time::pqc::PQCAlgorithm::DILITHIUM_3);
    EXPECT_FALSE(signature.empty());
    EXPECT_EQ(signature.size(), 3366);
    
    bool verified = pqc_algorithms_->dilithium_verify(message_bytes, signature, keypair.public_key, time::pqc::PQCAlgorithm::DILITHIUM_3);
...
TEST_F(SecurityFeaturesTest, PQCHybridEncryption) {
    ...
    auto encrypted = pqc_algorithms_->hybrid_encrypt(plaintext_bytes, keypair.public_key, time::pqc::PQCAlgorithm::KYBER_768);
    ...
    EXPECT_EQ(encrypted.pqc_algorithm, time::pqc::PQCAlgorithm::KYBER_768);
    
    auto decrypted = pqc_algorithms_->hybrid_decrypt(encrypted, keypair.private_key, time::pqc::PQCAlgorithm::KYBER_768);
    EXPECT_EQ(decrypted, plaintext_bytes);
}
...
TEST_F(SecurityFeaturesTest, PQCPerformance) {
    ...
    for (int i = 0; i < 100; i++) {
        auto encrypted = pqc_algorithms_->hybrid_encrypt(test_data, keypair.public_key, time::pqc::PQCAlgorithm::KYBER_768);
        auto decrypted = pqc_algorithms_->hybrid_decrypt(encrypted, keypair.private_key, time::pqc::PQCAlgorithm::KYBER_768);
...
TEST_F(SecurityFeaturesTest, PQCKeyUniqueness) {
    auto keypair1 = pqc_algorithms_->generate_kyber_keypair(time::pqc::PQCAlgorithm::KYBER_768);
    auto keypair2 = pqc_algorithms_->generate_kyber_keypair(time::pqc::PQCAlgorithm::KYBER_768);
...
TEST_F(SecurityFeaturesTest, PQCEncryptionUniqueness) {
    ...
    auto encrypted1 = pqc_algorithms_->hybrid_encrypt(plaintext_bytes, keypair.public_key, time::pqc::PQCAlgorithm::KYBER_768);
    auto encrypted2 = pqc_algorithms_->hybrid_encrypt(plaintext_bytes, keypair.public_key, time::pqc::PQCAlgorithm::KYBER_768);
