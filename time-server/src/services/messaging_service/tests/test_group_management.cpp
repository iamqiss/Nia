...
protected:
    ...
    time::mls::KeyPackage createTestKeyPackage() {
...
TEST_F(GroupManagementTest, GroupCreation) {
    ...
    EXPECT_EQ(status, time::mls::GroupSizeStatus::OPTIMAL);
}
...
TEST_F(GroupManagementTest, AddMembersBeyondOptimal) {
    ...
    for (uint32_t i = 0; i < 400; i++) {
        ...
        if (member_count <= 250) {
            ...
            EXPECT_EQ(status, time::mls::GroupSizeStatus::GOOD);
        }
    }
}
...
TEST_F(GroupManagementTest, AddMembersToMaximum) {
    ...
    for (uint32_t i = 0; i < 500; i++) {
        ...
        if (member_count <= 250) {
            ...
            EXPECT_EQ(status, time::mls::GroupSizeStatus::WARNING);
        } else if (member_count == 500) {
            EXPECT_EQ(status, time::mls::GroupSizeStatus::AT_LIMIT);
        }
    }
...
TEST_F(GroupManagementTest, GroupSizeStatusTransitions) {
    ...
    std::vector<std::pair<uint32_t, time::mls::GroupSizeStatus>> test_cases = {
        {0, time::mls::GroupSizeStatus::OPTIMAL},
        {100, time::mls::GroupSizeStatus::OPTIMAL},
        {250, time::mls::GroupSizeStatus::OPTIMAL},
        {251, time::mls::GroupSizeStatus::GOOD},
        {400, time::mls::GroupSizeStatus::GOOD},
        {401, time::mls::GroupSizeStatus::WARNING},
        {499, time::mls::GroupSizeStatus::WARNING},
        {500, time::mls::GroupSizeStatus::AT_LIMIT},
    };
    
    for (const auto& test_case : test_cases) {
        uint32_t target_count = test_case.first;
        time::mls::GroupSizeStatus expected_status = test_case.second;
