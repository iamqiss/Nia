...
router.post('/follow/:userId', requireAuth, async (req, res, next) => {
  try {
    ...
    if (userId === req.auth!.userId) throw new ValidationError('Cannot follow yourself');
...
router.post('/block/:userId', requireAuth, async (req, res, next) => {
  try {
    const { userId } = parseWithZod(idParam, req.params);
    if (userId === req.auth!.userId) throw new ValidationError('Cannot block yourself');
...
router.post('/mute/:userId', requireAuth, async (req, res, next) => {
  try {
    const { userId } = parseWithZod(idParam, req.params);
    if (userId === req.auth!.userId) throw new ValidationError('Cannot mute yourself');
