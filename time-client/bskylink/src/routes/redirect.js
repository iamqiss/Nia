...
const INTERNAL_IP_REGEX = new RegExp('(^127.[0-9]{1,3}.[0-9]{1,3}.[0-9]{1,3}$)|(^10.[0-9]{1,3}.[0-9]{1,3}.[0-9]{1,3}$)|(^172.1[6-9]{1}[0-9]{0,1}.[0-9]{1,3}.[0-9]{1,3}$)|(^172.2[0-9]{1}[0-9]{0,1}.[0-9]{1,3}.[0-9]{1,3}$)|(^172.3[0-1]{1}[0-9]{0,1}.[0-9]{1,3}.[0-9]{1,3}$)|(^192.168.[0-9]{1,3}.[0-9]{1,3}$)|^localhost', 'i');
export default function (ctx, app) {
    return app.get('/redirect', handler(async (req, res) => {
            ...
            if (rule !== 'ok') {
                switch (rule.action) {
                    case 'whitelist':
                        ...
                        break;
                    case 'block':
                        html = linkWarningLayout('Blocked Link Warning', linkWarningContents(req, {
                            type: 'block',
                            link: url.href,
                        }));
                        res.setHeader('Cache-Control', 'no-store');
                        ...
                        break;
                    case 'warn':
                        html = linkWarningLayout('Malicious Link Warning', linkWarningContents(req, {
                            type: 'warn',
                            link: url.href,
                        }));
                        res.setHeader('Cache-Control', 'no-store');
