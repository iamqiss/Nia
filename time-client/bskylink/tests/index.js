...
describe('link service', async () => {
    ...
    before(async () => {
        ...
        const { port } = linkService.server?.address();
        ...
        linkService.ctx.cfg.eventCache.smartUpdate({
            ...
            createdBy: 'did:example:admin',
            createdAt: later,
            comment: 'BONES has been resurrected to bring good music to the world once again',
        });
        linkService.ctx.cfg.eventCache.smartUpdate({
            $type: 'tools.ozone.safelink.defs#event',
            id: 6,
            ...
            createdBy: 'did:example:admin',
            createdAt: now,
            comment: 'Could be quite the mistake to get into this addicting game, but we will warn instead of block',
        });
    });
    after(async () => {
    ...
    it('League of Legends warned', async () => {
        ...
        assert.match(html, new RegExp(urlToRedirect.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')));
        // League of Legends is set to WARN, not BLOCK, so expect a warning (blocked-site div present)
        assert.match(html, /Warning: Malicious Link/, 'Expected warning not found in HTML');
    });
    ...
    it('Unsafe redirect with block rule, due to the content of webpage.', async () => {
        const urlToRedirect = 'https://gist.github.com/MattIPv4/045239bc27b16b2bcf7a3a9a4648c08a';
        const url = new URL(`${baseUrl}/redirect`);
        url.searchParams.set('u', urlToRedirect);
        const res = await fetch(url, { redirect: 'manual' });
        assert.strictEqual(res.status, 200);
        const html = await res.text();
        assert.match(html, new RegExp(urlToRedirect.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')));
        assert.match(html, /"blocked-site"/, 'Expected blocked-site div not found in HTML');
    });
    it('Rule adjustment, safe redirect, 200 response for Instagram Account of teamsesh Bones', async () => {
        ...
        const result = linkService.ctx.cfg.eventCache.smartGet('https://www.instagram.com/teamseshbones/?hl=en');
    ...
    async function getRedirect(link) {
        const url = new URL(link);
    ...
    async function getJsonRedirect(link) {
        const url = new URL(link);
        const base = new URL(baseUrl);
        url.protocol = base.protocol;
        url.host = base.host;
