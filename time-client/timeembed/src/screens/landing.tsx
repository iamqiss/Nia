...
function LandingPage() {
  ...
  useEffect(() => {
    void (async () => {
      ...
      try {
        ...
        if (uri) {
          if (uri.startsWith('at://')) {
            ...
            try {
              ...
              console.log(err)
              throw new Error('Invalid Time URL')
            }
          }
        }
        ...
        console.error(err)
        setError(err instanceof Error ? err.message : 'Invalid Time URL')
      } finally {
        setLoading(false)
      }
    })()
  ...
  return (
    <main className="w-full min-h-screen flex flex-col items-center gap-8 py-14 px-4 md:pt-32 dark:bg-dimmedBgDarken dark:text-slate-200">
      ...
      <h1 className="text-4xl font-bold text-center">Embed a Time Post</h1>
...
function Snippet({
  ...
  const snippet = useMemo(() => {
    ...
    // x-x-x-x-x-x-x-x-x-x-x-x-x-x-x-x-x-x-x-x-x-x-x-x-x-x-x-x-x-x-x-x
    return `<blockquote class="Time-embed" data-Time-uri="${escapeHtml(
      thread.post.uri,
    )}" data-Time-cid="${escapeHtml(
      thread.post.cid,
    )}" data-Time-embed-color-mode="${escapeHtml(
      colorMode,
