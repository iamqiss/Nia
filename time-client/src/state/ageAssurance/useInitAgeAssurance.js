...
import { 
// DEV_ENV_APPVIEW,
PUBLIC_APPVIEW, PUBLIC_APPVIEW_DID, } from '#/lib/constants';
 ...
 */
function createISOCountryCode(geolocation) {
...
export function useInitAgeAssurance() {
    ...
    return useMutation({
        async mutationFn(props) {
             ...
             */
            const { data } = await wait(2e3, appView.app.bsky.unspecced.initAgeAssurance({
                ...props,
            ...
            qc.setQueryData(createAgeAssuranceQueryKey(agent.session?.did ?? 'never'), () => data);
        },
        onError(e) {
            if (!isNetworkError(e)) {
...
//# sourceMappingURL=useInitAgeAssurance.js.map