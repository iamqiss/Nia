...
import {} from '#/state/ageAssurance/types';
import { useIsAgeAssuranceEnabled } from '#/state/ageAssurance/useIsAgeAssuranceEnabled';
import { logger } from '#/state/ageAssurance/util';
import { useGeolocationStatus } from '#/state/geolocation';
import { useAgent } from '#/state/session';
export const createAgeAssuranceQueryKey = (did) => ['ageAssurance', did];
const DEFAULT_AGE_ASSURANCE_STATE = {
...
export function Provider({ children }) {
    ...
    const { data, isFetched, refetch } = useQuery({
        ...
        async queryFn() {
            if (!agent.session)
                return null;
            try {
                const { data } = await networkRetry(3, () => agent.app.bsky.unspecced.getAgeAssuranceState());
                // const {data} = {
                //   data: {
    ...
    if (!!ageAssuranceContext.lastInitiatedAt &&
    ...
    else if (!!ageAssuranceContext.lastInitiatedAt &&
        ageAssuranceContext.status !== 'pending' &&
        refetchWhilePending) {
    ...
    return (_jsx(AgeAssuranceAPIContext.Provider, { value: ageAssuranceAPIContext, children: _jsx(AgeAssuranceContext.Provider, { value: ageAssuranceContext, children: children }) }));
}
/**
