...
import { DEFAULT_GEOLOCATION_CONFIG, GEOLOCATION_CONFIG_URL, } from '#/state/geolocation/const';
...
async function getGeolocationConfig(url) {
 ...
 */
let geolocationConfigResolution;
/**
    ...
    geolocationConfigResolution = new Promise(async (resolve) => {
        ...
        try {
            ...
            if (config) {
                ...
                throw new Error(`geolocation config: nothing returned from initial request`);
            }
        }
        catch (e) {
            success = false;
