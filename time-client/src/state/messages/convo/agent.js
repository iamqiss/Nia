...
import {} from '#/state/messages/events/agent';
import {} from '#/state/messages/events/types';
...
export class Convo {
    ...
    async fetchConvo() {
        ...
        this.pendingFetchConvo = new Promise(async (resolve, reject) => {
            try {
    ...
    cleanupFirehoseConnection;
    setupFirehose() {
        // remove old listeners, if exist
        this.cleanupFirehoseConnection?.();
        // reconnect
        this.cleanupFirehoseConnection = this.events.on(event => {
            switch (event.type) {
                case 'connect': {
    ...
    onFirehoseError(error) {
        this.firehoseError = error;
        this.commit();
    }
    ingestFirehose(events) {
    ...
    getItems() {
        ...
        this.pendingMessages.forEach(m => {
            items.push({
                ...
                retry: this.pendingMessageFailure === 'recoverable'
                    ? () => {
                        this.maybeRecoverFromNetworkError();
                    }
                    : undefined,
    ...
    async addReaction(messageId, emoji) {
        ...
        if (this.pastMessages.has(messageId)) {
            ...
            if (ChatBskyConvoDefs.isMessageView(prevMessage) &&
                ...
                !prevMessage.reactions?.find(reaction => reaction.sender.did === this.senderUserDid &&
                    reaction.value === emoji)) {
                if (prevMessage.reactions) {
                    if (prevMessage.reactions.filter(reaction => reaction.sender.did === this.senderUserDid).length >= 5) {
        ...
        try {
            ...
            const { data } = await this.agent.chat.bsky.convo.addReaction({ messageId, value: emoji, convoId: this.convoId }, { encoding: 'application/json', headers: DM_SERVICE_HEADERS });
    ...
    async removeReaction(messageId, emoji) {
        ...
        if (this.pastMessages.has(messageId)) {
            ...
            if (ChatBskyConvoDefs.isMessageView(prevMessage)) {
                this.pastMessages.set(messageId, {
                    ...prevMessage,
                    reactions: prevMessage.reactions?.filter(reaction => reaction.value !== emoji ||
        ...
        try {
            ...
            await this.agent.chat.bsky.convo.removeReaction({ messageId, value: emoji, convoId: this.convoId }, { encoding: 'application/json', headers: DM_SERVICE_HEADERS });
        }
        catch (error) {
            if (restore)
                restore();
