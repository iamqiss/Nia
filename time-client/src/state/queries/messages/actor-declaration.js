...
export function useUpdateActorDeclaration({ onSuccess, onError, }) {
    const queryClient = useQueryClient();
    const { currentAccount } = useSession();
    const agent = useAgent();
    return useMutation({
        mutationFn: async (allowIncoming) => {
            if (!currentAccount)
        ...
        onMutate: allowIncoming => {
            if (!currentAccount)
                return;
            queryClient.setQueryData(PROFILE_RKEY(currentAccount?.did), (old) => {
                if (!old)
                    return old;
                return {
                    ...old,
...
//# sourceMappingURL=actor-declaration.js.map