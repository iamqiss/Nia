...
export function useLeaveConvo(convoId, { onSuccess, onMutate, onError, }) {
    ...
    return useMutation({
        ...
        mutationFn: async () => {
            ...
            const { data } = await agent.chat.bsky.convo.leaveConvo({ convoId }, { headers: DM_SERVICE_HEADERS, encoding: 'application/json' });
            return data;
        },
        onMutate: () => {
            let prevPages = [];
            queryClient.setQueryData([CONVO_LIST_KEY], (old) => {
                if (!old)
                    return old;
                prevPages = old.pages;
                return {
...
//# sourceMappingURL=leave-conversation.js.map