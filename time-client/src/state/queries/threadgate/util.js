...
export function threadgateViewToAllowUISetting(threadgateView) {
    ...
    const threadgate = threadgateView &&
...
export function threadgateRecordToAllowUISetting(threadgate) {
    ...
    const settings = threadgate.allow
        .map(allow => {
        let setting;
...
export function mergeThreadgateRecords(prev, next) {
    ...
    const allow = prev.allow || next.allow
        ? [...(prev.allow || []), ...(next.allow || [])].filter((v, i, a) => a.findIndex(t => t.$type === v.$type) === i)
        : undefined;
