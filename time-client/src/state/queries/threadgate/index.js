...
import {} from '#/state/queries/threadgate/types';
...
export const createThreadgateRecordQueryKey = (uri) => [
    threadgateRecordQueryKeyRoot,
    uri,
];
export function useThreadgateRecordQuery({ postUri, initialData, } = {}) {
...
export const createThreadgateViewQueryKey = (uri) => [
    threadgateViewQueryKeyRoot,
    uri,
];
export function useThreadgateViewQuery({ postUri, initialData, } = {}) {
    const agent = useAgent();
    return useQuery({
        enabled: !!postUri,
...
export async function getThreadgateView({ agent, postUri, }) {
...
export async function getThreadgateRecord({ agent, postUri, }) {
...
export async function writeThreadgateRecord({ agent, postUri, threadgate, }) {
    ...
    await networkRetry(2, () => agent.api.com.atproto.repo.putRecord({
...
export async function upsertThreadgate({ agent, postUri, }, callback) {
 ...
 */
export async function updateThreadgateAllow({ agent, postUri, allow, }) {
...
export function useSetThreadgateAllowMutation() {
    ...
    return useMutation({
        mutationFn: async ({ postUri, allow, }) => {
            return upsertThreadgate({ agent, postUri }, async (prev) => {
                if (prev) {
                    return {
                        ...prev,
...
export function useToggleReplyVisibilityMutation() {
    ...
    return useMutation({
        mutationFn: async ({ postUri, replyUri, action, }) => {
            if (action === 'hide') {
