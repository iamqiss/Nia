...
export const labelerInfoQueryKey = (did) => [
    labelerInfoQueryKeyRoot,
    did,
];
const labelersInfoQueryKeyRoot = 'labelers-info';
export const labelersInfoQueryKey = (dids) => [
    labelersInfoQueryKeyRoot,
    dids.slice().sort(),
];
export const labelersDetailedInfoQueryKey = (dids) => [
    labelersDetailedInfoQueryKeyRoot,
    dids,
];
export function useLabelerInfoQuery({ did, enabled, }) {
...
export function useLabelersInfoQuery({ dids }) {
    const agent = useAgent();
    return useQuery({
        enabled: !!dids.length,
...
export function useLabelersDetailedInfoQuery({ dids }) {
    const agent = useAgent();
    return useQuery({
        enabled: !!dids.length,
...
export function useLabelerSubscriptionMutation() {
    ...
    return useMutation({
        async mutationFn({ did, subscribe }) {
            // TODO
            z.object({
                did: z.string(),
                subscribe: z.boolean(),
