...
export function createCacheMutator({ queryClient, postThreadQueryKey, postThreadOtherQueryKey, params, }) {
    return {
        insertReplies(parentUri, replies) {
            /*
             * Main thread query mutator.
             */
            queryClient.setQueryData(postThreadQueryKey, data => {
                if (!data)
                    return;
                return {
                    ...data,
             ...
             */
            queryClient.setQueryData(postThreadOtherQueryKey, data => {
                if (!data)
                    return;
                return {
                    ...data,
            ...
            function mutator(thread) {
                for (let i = 0; i < thread.length; i++) {
                    ...
                    if (!AppBskyUnspeccedDefs.isThreadItemPost(parent.value))
                        continue;
                     ...
                     */
                    const canAlwaysInsertReplies = isParentRoot ||
         ...
         */
        deletePost(post) {
            queryClient.setQueryData(postThreadQueryKey, queryData => {
                if (!queryData)
                    return;
