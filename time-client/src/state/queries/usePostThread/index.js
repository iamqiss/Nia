...
export function usePostThread({ anchor }) {
    ...
    const { isLoaded: isThreadPreferencesLoaded, sort, setSort: baseSetSort, view, setView: baseSetView, prioritizeFollowedUsers, } = useThreadPreferences();
    const below = useMemo(() => {
        return view === 'linear'
            ? LINEAR_VIEW_BELOW
            : isWeb && gtPhone
     ...
     */
    const mutator = useMemo(() => createCacheMutator({
        params: { view, below },
        postThreadQueryKey,
    ...
    const serverOtherThreadItems = useMemo(() => {
        if (!additionalQueryEnabled)
            return [];
        if (additionalItemsQuery.isLoading) {
            return Array.from({ length: 2 }).map((_, i) => views.skeleton({
                key: `other-reply-${i}`,
                item: 'reply',
            }));
        }
