...
export function sortAndAnnotateThreadItems(thread, { threadgateHiddenReplies, moderationOpts, view, skipModerationHandling, }) {
    const threadItems = [];
    const otherThreadItems = [];
    ...
    traversal: for (let i = 0; i < thread.length; i++) {
        ...
        if (item.depth < 0) {
             ...
             * we could.
             */
            const shouldBreak = AppBskyUnspeccedDefs.isThreadItemNoUnauthenticated(item.value) ||
                    ...
                    if (parentIsTopLevelReply) {
                        ...
                        for (let ci = startIndex; ci <= branch.end; ci++) {
                            ...
                            if (AppBskyUnspeccedDefs.isThreadItemPost(child.value)) {
                                const childParentMetadata = metadatas.get(getPostRecord(child.value.post).reply?.parent?.uri || '');
            ...
            if (item.type === 'threadPost') {
                ...
                if (metadata) {
                    if (metadata.parentMetadata) {
                         ...
                         * own reply branches).
                         */
                        const isLastSiblingByCounts = metadata.replyIndex ===
                         ...
                         * item).
                         */
                        const isImplicitlyLastSibling = metadata.nextItemDepth === undefined ||
...
export function buildThread({ threadItems, otherThreadItems, serverOtherThreadItems, isLoading, hasSession, otherItemsVisible, hasOtherThreadItems, showOtherItems, }) {
    /**
    ...
    if (isLoading) {
        ...
        if (hasSession) {
            ...
            const replyDisabled = hasAnchorFromCache &&
