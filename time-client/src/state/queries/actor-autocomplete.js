...
export const RQKEY = (prefix) => [RQKEY_ROOT, prefix];
...
export function useActorAutocompleteFn() {
    ...
    const moderationOpts = useModerationOpts();
    const agent = useAgent();
    return React.useCallback(async ({ query, limit = 8 }) => {
        ...
        if (query) {
            try {
                res = await queryClient.fetchQuery({
                    ...
                    queryFn: () => agent.searchActorsTypeahead({
                        q: query,
                        limit,
                    }),
                });
...
function computeSuggestions({ q, searched = [], moderationOpts, }) {
    let items = [];
...
//# sourceMappingURL=actor-autocomplete.js.map