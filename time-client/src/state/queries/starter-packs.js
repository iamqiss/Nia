import { AppBskyGraphDefs, AppBskyGraphStarterpack, AtUri, RichText, } from '@atproto/api';
...
export function useStarterPackQuery({ uri, did, rkey, }) {
...
export async function invalidateStarterPack({ queryClient, did, rkey, }) {
    await queryClient.invalidateQueries({ queryKey: RQKEY({ did, rkey }) });
}
export function useCreateStarterPackMutation({ onSuccess, onError, }) {
    const queryClient = useQueryClient();
    const agent = useAgent();
    return useMutation({
        mutationFn: async ({ name, description, feeds, profiles }) => {
            let descriptionFacets;
            if (description) {
...
export function useEditStarterPackMutation({ onSuccess, onError, }) {
    const queryClient = useQueryClient();
    const agent = useAgent();
    return useMutation({
        mutationFn: async ({ name, description, feeds, profiles, currentStarterPack, currentListItems, }) => {
            ...
            const removedItems = currentListItems.filter(i => i.subject.did !== agent.session?.did &&
...
export function useDeleteStarterPackMutation({ onSuccess, onError, }) {
    const agent = useAgent();
    const queryClient = useQueryClient();
    return useMutation({
        mutationFn: async ({ listUri, rkey }) => {
            if (!agent.session) {
...
export async function precacheStarterPack(queryClient, starterPack) {
