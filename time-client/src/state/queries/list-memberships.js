    ...
    return useQuery({
        ...
        async queryFn() {
            ...
            for (let i = 0; i < SANITY_PAGE_LIMIT; i++) {
                ...
                arr = arr.concat(res.records.map(r => ({
                    membershipUri: r.uri,
...
export function useListMembershipAddMutation({ onSuccess, onError, } = {}) {
    const { currentAccount } = useSession();
    const agent = useAgent();
    ...
    return useMutation({
        mutationFn: async ({ listUri, actorDid }) => {
            ...
            const res = await agent.app.bsky.graph.listitem.create({ repo: currentAccount.did }, {
                subject: actorDid,
                list: listUri,
        ...
        onSuccess: (data, variables) => {
            ...
            if (memberships) {
                memberships = memberships
                    // avoid dups
                    .filter(m => !(m.actorDid === variables.actorDid &&
...
export function useListMembershipRemoveMutation({ onSuccess, onError, } = {}) {
    const { currentAccount } = useSession();
    const agent = useAgent();
    const queryClient = useQueryClient();
    return useMutation({
        ...
        onSuccess: (data, variables) => {
            ...
            let memberships = queryClient.getQueryData(RQKEY());
            if (memberships) {
                memberships = memberships.filter(m => !(m.actorDid === variables.actorDid &&
                    m.listUri === variables.listUri));
                queryClient.setQueryData(RQKEY(), memberships);
            }
            // invalidate the members queries (used for rendering the listings)
...
//# sourceMappingURL=list-memberships.js.map