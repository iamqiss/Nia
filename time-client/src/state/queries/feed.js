...
export const feedSourceInfoQueryKey = ({ uri }) => [
...
export function useFeedSourceInfoQuery({ uri }) {
                ...
                pages: data.pages.map(page => {
                    return {
                        ...
                        feeds: page.feeds.filter(feed => {
                            ...
                            const alreadySaved = Boolean(savedFeeds?.find(f => {
...
export const createPopularFeedsSearchQueryKey = (query) => [
    popularFeedsSearchQueryKeyRoot,
    query,
];
export function usePopularFeedsSearch({ query, enabled, }) {
    const agent = useAgent();
    const moderationOpts = useModerationOpts();
...
export function usePinnedFeedsInfos() {
    ...
    return useQuery({
        ...
        queryFn: async () => {
            ...
            const listsPromises = pinnedLists.map(list => agent.app.bsky.graph
                .getList({
                list: list.value,
                limit: 1,
            })
...
export function useSavedFeeds() {
    ...
    return useQuery({
        ...
        queryFn: async () => {
            ...
            const listsPromises = savedLists.map(list => agent.app.bsky.graph
                .getList({
                list: list.value,
                limit: 1,
            })
