...
import {} from '#/lib/api/feed/types';
    ...
    const query = useInfiniteQuery({
        ...
        async queryFn({ pageParam }) {
            ...
            try {
                 ...
                 */
                if (!agent.session) {
                    assertSomePostsPassModeration(res.feed, preferences?.moderationPrefs ||
                        ...
                        slices: tuner
                            ...
                            for (let i = 0; i < slice.items.length; i++) {
                                ...
                                if (ignoreFilter) {
                                    ...
                                    moderations[i].causes = moderations[i].causes.filter(cause => cause.type !== 'muted');
                                }
                                if (!ignoreFilter &&
                            ...
                            if (isDiscover) {
                                userActionHistory.seen(slice.items.map(item => ({
...
function createApi({ feedDesc, feedParams, feedTuners, userInterests, agent, enableFollowingToDiscoverFallback, }) {
...
export function resetPostsFeedQueries(queryClient, timeout = 0) {
    setTimeout(() => {
...
export function resetProfilePostsQueries(queryClient, did, timeout = 0) {
    setTimeout(() => {
        queryClient.resetQueries({
            predicate: query => !!(query.queryKey[0] === RQKEY_ROOT &&
                query.queryKey[1]?.includes(did)),
        });
    }, timeout);
}
