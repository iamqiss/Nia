 ...
 */
export const DEFAULT_LOGGED_OUT_LABEL_PREFERENCES = Object.fromEntries(Object.entries(DEFAULT_LABEL_SETTINGS).map(([key, _pref]) => [key, 'hide']));
export function useMyLabelersQuery({ excludeNonConfigurableLabelers = false, } = {}) {
    const prefs = usePreferencesQuery();
    let dids = Array.from(new Set(BskyAgent.appLabelers.concat(prefs.data?.moderationPrefs.labelers.map(l => l.did) || [])));
...
export function useLabelDefinitionsQuery() {
    ...
    return React.useMemo(() => {
        return {
            labelDefs: Object.fromEntries((labelers.data || []).map(labeler => [
                labeler.creator.did,
