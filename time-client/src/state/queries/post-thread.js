...
export function usePostThreadQuery(uri) {
...
export function fillThreadModerationCache(cache, node, moderationOpts) {
    if (node.type === 'post') {
...
export function sortThread(node, opts, modCache, currentDid, justPostedUris, threadgateRecordHiddenReplies, fetchedAtCache, fetchedAt, randomCache) {
    if (node.type !== 'post') {
        return node;
    }
    if (node.replies) {
        ...
        node.replies.forEach(reply => sortThread(reply, opts, modCache, currentDid, justPostedUris, threadgateRecordHiddenReplies, fetchedAtCache, fetchedAt, randomCache));
    }
    return node;
}
// internal methods
...
function getHotness(threadPost, fetchedAt) {
    const { post, hasOPLike } = threadPost;
    const hoursAgo = Math.max(0, (new Date(fetchedAt).getTime() - new Date(post.indexedAt).getTime()) /
        (1000 * 60 * 60));
...
function responseToThreadNodes(node, depth = 0, direction = 'start') {
    if (AppBskyFeedDefs.isThreadViewPost(node) &&
        bsky.dangerousIsType(node.post.record, AppBskyFeedPost.isRecord)) {
        ...
        return {
            ...
            parent: node.parent && direction !== 'down'
                ? responseToThreadNodes(node.parent, depth - 1, 'up')
                : undefined,
            replies: node.replies?.length && direction !== 'up'
                ? node.replies
...
function annotateSelfThread(thread) {
    if (thread.type !== 'post') {
        return;
    }
    const selfThreadNodes = [thread];
...
function findPostInQueryData(queryClient, uri) {
    let partial;
...
export function* findAllProfilesInQueryData(queryClient, did) {
    ...
    for (let profile of findAllProfilesInExploreFeedPreviewsQueryData(queryClient, did)) {
        yield profile;
    }
}
function* traverseThread(node) {
    if (node.type === 'post') {
        if (node.parent) {
...
function threadNodeToPlaceholderThread(node) {
    if (node.type !== 'post') {
        return undefined;
    }
    return {
...
function postViewToPlaceholderThread(post) {
    return {
        type: 'post',
        _reactKey: post.uri,
        uri: post.uri,
        post: post,
        record: post.record, // validated in notifs
        parent: undefined,
        replies: undefined,
        hasOPLike: undefined,
        ctx: {
            ...
            isHighlightedPost: true,
            hasMore: false,
            isParentLoading: !!post.record.reply,
            isChildLoading: true, // assume yes (show the spinner) just in case
        },
    };
}
function embedViewRecordToPlaceholderThread(record) {
    return {
        type: 'post',
        _reactKey: record.uri,
        uri: record.uri,
        post: embedViewRecordToPostView(record),
        record: record.value, // validated in getEmbeddedPost
        parent: undefined,
        replies: undefined,
        hasOPLike: undefined,
        ctx: {
            ...
            isHighlightedPost: true,
            hasMore: false,
            isParentLoading: !!record.value.reply,
