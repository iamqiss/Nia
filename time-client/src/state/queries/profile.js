...
import { logEvent, toClout } from '#/lib/statsig/statsig';
...
export const profilesQueryKey = (handles) => [
    profilesQueryKeyRoot,
    handles,
];
export function useProfileQuery({ did, staleTime = STALE.SECONDS.FIFTEEN, }) {
...
export function useProfilesQuery({ handles, maintainData, }) {
    const agent = useAgent();
    return useQuery({
...
export function usePrefetchProfileQuery() {
    ...
    const prefetchProfileQuery = useCallback(async (did) => {
...
export function useProfileUpdateMutation() {
    ...
    return useMutation({
        mutationFn: async ({ profile, updates, newUserAvatar, newUserBanner, checkCommitted, }) => {
            ...
            await whenAppViewReady(agent, profile.did, checkCommitted ||
                (res => {
...
export function useProfileFollowMutationQueue(profile, logContext) {
    const agent = useAgent();
    const queryClient = useQueryClient();
...
function useProfileFollowMutation(logContext, profile) {
...
function useProfileBlockMutation() {
    ...
    return useMutation({
        mutationFn: async ({ did }) => {
            ...
            return await agent.app.bsky.graph.block.create({ repo: currentAccount.did }, { subject: did, createdAt: new Date().toISOString() });
        },
        onSuccess(_, { did }) {
...
async function whenAppViewReady(agent, actor, fn) {
    await until(5, // 5 tries
    1e3, // 1s delay between tries
    fn, () => agent.app.bsky.actor.getProfile({ actor }));
}
export function* findAllProfilesInQueryData(queryClient, did) {
    const profileQueryDatas = queryClient.getQueriesData({
        queryKey: [RQKEY_ROOT],
    });
    ...
    const profilesQueryDatas = queryClient.getQueriesData({
