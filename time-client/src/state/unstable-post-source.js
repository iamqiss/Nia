 ...
 */
const transientSources = new Map();
/**
...
export function setUnstablePostSource(key, source) {
    assertValidDevOnly(key, `setUnstablePostSource key should be a URI containing a handle, received ${key} — use buildPostSourceKey`);
...
export function useUnstablePostSource(key) {
    ...
    const [source] = useState(() => {
        assertValidDevOnly(key, `consumeUnstablePostSource key should be a URI containing a handle, received ${key} — be sure to use buildPostSourceKey when setting the source`, true);
 ...
 */
function assertValidDevOnly(key, message, beChill = false) {
    if (__DEV__) {
        const urip = new AtUri(key);
...
//# sourceMappingURL=unstable-post-source.js.map