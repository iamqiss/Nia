...
export function ToastConfigProvider({ children, id, type, }) {
    return (_jsx(ToastConfigContext.Provider, { value: useMemo(() => ({ id, type }), [id, type]), children: children }));
}
...
export function Action(props) {
    ...
    return (_jsx(View, { style: { top: fontScaleCompensation }, children: _jsx(Button, { ...props, onPress: onPress, children: s => {
...
function useToastFontScaleCompensation() {
    const { fonts } = useAlf();
    const fontScaleCompensation = useMemo(() => parseInt(fonts.scale) * -1 * 0.65, [fonts.scale]);
    return useMemo(() => ({
        fontScaleCompensation,
    }), [fontScaleCompensation]);
}
function useToastStyles({ type }) {
    const t = useTheme();
    return useMemo(() => {
        return {
            default: {
