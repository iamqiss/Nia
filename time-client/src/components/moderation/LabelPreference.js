...
export function Content({ children, name, description, }) {
    const t = useTheme();
    const { gtPhone } = useBreakpoints();
    return (_jsxs(View, { style: [a.gap_xs, a.flex_1], children: [_jsx(Text, { emoji: true, style: [a.font_bold, gtPhone ? a.text_sm : a.text_md], children: name }), _jsx(Text, { emoji: true, style: [t.atoms.text_contrast_medium, a.leading_snug], children: description }), children] }));
}
export function Buttons({ name, values, onChange, ignoreLabel, warnLabel, hideLabel, disabled, }) {
    const { _ } = useLingui();
    return (_jsx(View, { style: [{ minHeight: 35 }, a.w_full], children: _jsxs(ToggleButton.Group, { disabled: disabled, label: _(msg `Configure content filtering setting for category: ${name}`), values: values, onChange: onChange, children: [ignoreLabel && (_jsx(ToggleButton.Button, { name: "ignore", label: ignoreLabel, children: _jsx(ToggleButton.ButtonText, { children: ignoreLabel }) })), warnLabel && (_jsx(ToggleButton.Button, { name: "warn", label: warnLabel, children: _jsx(ToggleButton.ButtonText, { children: warnLabel }) })), hideLabel && (_jsx(ToggleButton.Button, { name: "hide", label: hideLabel, children: _jsx(ToggleButton.ButtonText, { children: hideLabel }) }))] }) }));
}
/**
 * For use on the global Moderation screen to set prefs for a "global" label,
 * not scoped to a single labeler.
 */
export function GlobalLabelPreference({ labelDefinition, disabled, }) {
    const { _ } = useLingui();
    ...
    const labelStrings = labelDefinition.identifier in allLabelStrings
    ...
    return (_jsxs(Outer, { children: [_jsx(Content, { name: labelStrings.name, description: labelStrings.description }), _jsx(Buttons, { name: labelStrings.name.toLowerCase(), values: [pref], onChange: values => {
                    mutate({
                        label: identifier,
                        visibility: values[0],
 ...
 */
export function LabelerLabelPreference({ labelDefinition, disabled, labelerDid, }) {
    ...
    return (_jsxs(Outer, { children: [_jsx(Content, { name: labelStrings.name, description: labelStrings.description, children: cantConfigure && (_jsxs(View, { style: [a.flex_row, a.gap_xs, a.align_center, a.mt_xs], children: [_jsx(CircleInfo, { size: "sm", fill: t.atoms.text_contrast_high.color }), _jsx(Text, { style: [t.atoms.text_contrast_medium, a.font_bold, a.italic], children: adultDisabled ? (_jsx(Trans, { children: "Adult content is disabled." })) : isGlobalLabel ? (_jsxs(Trans, { children: ["Configured in", ' ', _jsx(InlineLinkText, { label: _(msg `moderation settings`), to: "/moderation", style: a.text_sm, children: "moderation settings" }), "."] })) : null })] })) }), showConfig && (_jsx(_Fragment, { children: cantConfigure ? (_jsx(View, { style: [
                        { minHeight: 35 },
                        a.px_md,
                        a.py_md,
                        a.rounded_sm,
                        ...
                        a.self_start,
                    ], children: _jsx(Text, { emoji: true, style: [a.font_bold, t.atoms.text_contrast_low], children: currentPrefLabel }) })) : (_jsx(Buttons, { name: labelStrings.name.toLowerCase(), values: [pref], onChange: values => {
                        mutate({
                            label: identifier,
                            visibility: values[0],
                            labelerDid,
...
//# sourceMappingURL=LabelPreference.js.map