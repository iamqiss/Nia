...
let MessageItem = ({ item, }) => {
    ...
    const appliedReactions = (_jsx(LayoutAnimationConfig, { skipEntering: true, skipExiting: true, children: message.reactions && message.reactions.length > 0 && (_jsx(View, { style: [isFromSelf ? a.align_end : a.align_start, a.px_sm, a.pb_2xs], children: _jsx(View, { style: [
                    a.flex_row,
                    a.gap_2xs,
                    a.py_xs,
                    a.px_xs,
                ...
                ], children: message.reactions.map((reaction, _i, reactions) => {
                    let label;
    ...
    return (_jsxs(_Fragment, { children: [isNewDay && _jsx(DateDivider, { date: message.sentAt }), _jsxs(View, { style: [
                    ...
                    nextIsMessage && !isNextFromSameSender && a.mb_md,
                                ...
                                ], children: _jsx(RichText, { value: rt, style: [a.text_md, isFromSelf && { color: t.palette.white }], interactiveStyle: a.underline, enableTags: true, emojiMultiplier: 3, shouldProxyLinks: true }) })), isNative && appliedReactions] }), !isNative && appliedReactions, isLastInGroup && (_jsx(MessageItemMetadata, { item: item, style: isFromSelf ? a.text_right : a.text_left }))] })] }));
};
MessageItem = React.memo(MessageItem);
export { MessageItem };
let MessageItemMetadata = ({ item, style, }) => {
    const t = useTheme();
    const { _ } = useLingui();
    const { message } = item;
    ...
    const relativeTimestamp = useCallback((i18n, timestamp) => {
    ...
    return (_jsxs(Text, { style: [
            ...
            style,
        ], children: [_jsx(TimeElapsed, { timestamp: message.sentAt, timeToString: relativeTimestamp, children: ({ timeElapsed }) => (_jsx(Text, { style: [a.text_xs, t.atoms.text_contrast_medium], children: timeElapsed })) }), item.type === 'pending-message' && item.failed && (_jsxs(_Fragment, { children: [' ', "\u00B7", ' ', _jsx(Text, { style: [
                            a.text_xs,
                            {
                                color: t.palette.negative_400,
                            },
                        ], children: _(msg `Failed to send`) }), item.retry && (_jsxs(_Fragment, { children: [' ', "\u00B7", ' ', _jsx(InlineLinkText, { label: _(msg `Click to retry failed message`), to: "#", onPress: handleRetry, style: [a.text_xs], children: _(msg `Retry`) })] }))] }))] }));
};
