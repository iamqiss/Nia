...
export function EmojiReactionPicker({ message, onEmojiSelect, }) {
    const { _ } = useLingui();
    ...
    return (_jsxs(View, { onLayout: evt => setLayout(evt.nativeEvent.layout), style: [
            bgColor,
            a.rounded_full,
            a.absolute,
            { bottom: '100%' },
            ...
            a.shadow_md,
                ...
                return (_jsx(ContextMenu.Item, { position: position, label: _(msg `React with ${emoji}`), onPress: () => onEmojiSelect(emoji), unstyled: true, disabled: limitReacted ? !alreadyReacted : false, children: hovered => (_jsx(View, { style: [
                            a.rounded_full,
                            hovered
                                ? {
            ...
            }), _jsx(EmojiPopup, { onEmojiSelected: emoji => {
                    close();
                    onEmojiSelect(emoji);
...
//# sourceMappingURL=EmojiReactionPicker.js.map