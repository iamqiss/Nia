...
export function Text({ children, emoji, style, selectable, title, dataSet, ...rest }) {
    ...
    return (_jsx(UITextView, { ...shared, children: renderChildrenWithEmoji(children, shared, emoji ?? false) }));
}
function createHeadingElement({ level }) {
    return function HeadingElement({ style, ...rest }) {
        const attr = web({
            role: 'heading',
            'aria-level': level,
        }) || {};
        return _jsx(Text, { ...attr, ...rest, style: style });
    };
}
/*
...
export function P({ style, ...rest }) {
    ...
    return (_jsx(Text, { ...attr, ...rest, style: [atoms.text_md, atoms.leading_normal, flatten(style)] }));
}
