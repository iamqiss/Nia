...
export function SearchablePeopleList({ title, showRecentConvos, sortByMessageDeclaration, onSelectChat, renderProfileCard, }) {
    const t = useTheme();
    const { _ } = useLingui();
    ...
    const listHeader = useMemo(() => {
        return (_jsxs(View, { onLayout: evt => setHeaderHeight(evt.nativeEvent.layout.height), style: [
                ...
                t.atoms.bg,
                            ...
                            ], children: title }), isWeb ? (_jsx(Button, { label: _(msg `Close`), size: "small", shape: "round", variant: isWeb ? 'ghost' : 'solid', color: "secondary", style: [
                                a.absolute,
                                a.z_20,
                                web({ right: -4 }),
                                native({ right: 0 }),
                                native({ height: 32, width: 32, borderRadius: 16 }),
                            ], onPress: () => control.close(), children: _jsx(ButtonIcon, { icon: X, size: "md" }) })) : null] }), _jsx(View, { style: web([a.pt_xs]), children: _jsx(SearchInput, { inputRef: inputRef, value: searchText, onChangeText: text => {
                            setSearchText(text);
...
function DefaultProfileCard({ profile, moderationOpts, onPress, }) {
    const t = useTheme();
    const { _ } = useLingui();
    ...
    return (_jsx(Button, { disabled: !enabled, label: _(msg `Start chat with ${displayName}`), onPress: handleOnPress, children: ({ hovered, pressed, focused }) => (_jsx(View, { style: [
                a.flex_1,
                a.py_sm,
                a.px_lg,
                !enabled
...
function SearchInput({ value, onChangeText, onEscape, inputRef, }) {
    const t = useTheme();
    const { _ } = useLingui();
    ...
    return (_jsxs(View, { ...web({
            ...
            , { 
                // @ts-ignore bottom sheet input types issue — esb
                ref: inputRef, placeholder: _(msg `Search`), value: value, onChangeText: onChangeText, onFocus: onFocus, onBlur: onBlur, style: [a.flex_1, a.py_md, a.text_md, t.atoms.text], placeholderTextColor: t.palette.contrast_500, keyboardAppearance: t.name === 'light' ? 'light' : 'dark', returnKeyType: "search", clearButtonMode: "while-editing", maxLength: 50, onKeyPress: ({ nativeEvent }) => {
                    if (nativeEvent.key === 'Escape') {
                        onEscape();
                    }
                }, autoCorrect: false, autoComplete: "off", autoCapitalize: "none", autoFocus: true, accessibilityLabel: _(msg `Search profiles`), accessibilityHint: _(msg `Searches for profiles`) })] }));
}
//# sourceMappingURL=SearchablePeopleList.js.map