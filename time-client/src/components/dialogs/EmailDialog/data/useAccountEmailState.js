...
export const accountEmailStateQueryKey = ['accountEmailState'];
export function useAccountEmailState() {
    const agent = useAgent();
    const { partialRefreshSession } = useSessionApi();
    const [prevIsEmailVerified, setPrevEmailIsVerified] = useState(!!agent.session?.emailConfirmed);
    const state = useMemo(() => ({
...
//# sourceMappingURL=useAccountEmailState.js.map