...
export function RecentChats({ postUri }) {
    ...
    if (!moderationOpts)
        return null;
    return (_jsxs(View, { style: [a.relative, a.flex_1, { marginHorizontal: tokens.space.md * -1 }], children: [_jsx(ScrollView, { horizontal: true, style: [a.flex_1, a.pt_2xs, { minHeight: 98 }], contentContainerStyle: [a.gap_sm, a.px_md], showsHorizontalScrollIndicator: false, fadingEdgeLength: 64, nestedScrollEnabled: true, children: convos && convos.length > 0 ? (convos.map(convo => {
                    const otherMember = convo.members.find(member => member.did !== currentAccount?.did);
                    if (!otherMember ||
                        otherMember.handle === 'missing.invalid' ||
                        convo.muted)
                        return null;
                    return (_jsx(RecentChatItem, { profile: otherMember, onPress: () => onSelectChat(convo.id), moderationOpts: moderationOpts }, convo.id));
...
const WIDTH = 80;
function RecentChatItem({ profile: profileUnshadowed, onPress, moderationOpts, }) {
    const { _ } = useLingui();
    const t = useTheme();
...
//# sourceMappingURL=RecentChats.js.map