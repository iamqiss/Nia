...
export function Root({ children }) {
    ...
    const hoverables = useRef(new Map());
    const hoverablesSV = useSharedValue({});
    const syncHoverablesThrottleRef = useRef(undefined);
    const [hoveredMenuItem, setHoveredMenuItem] = useState(null);
    const onHoverableTouchUp = useCallback((id) => {
    ...
    const context = useMemo(() => ({
        ...
        close: () => {
            animationSV.set(withSpring(0, SPRING_OUT, finished => {
                if (finished) {
                    hoverablesSV.set({});
                    translationSV.set(0);
        ...
        registerHoverable: (id, rect, onTouchUp) => {
...
export function Trigger({ children, label, contentLabel, style }) {
    ...
    const open = useNonReactiveCallback(async (mode) => {
        playHaptic();
        Keyboard.dismiss();
        const [measurement, capture] = await Promise.all([
            new Promise(resolve => {
                ref.current?.measureInWindow((x, y, width, height) => resolve({
                    x,
                    y: y +
                        platform({
                            default: 0,
    ...
    const { hoverablesSV, setHoveredMenuItem, onTouchUpMenuItem, translationSV, animationSV, } = context;
    const hoveredItemSV = useSharedValue(null);
    useAnimatedReaction(() => hoveredItemSV.get(), (hovered, prev) => {
        if (hovered !== prev) {
    ...
    return (_jsxs(_Fragment, { children: [_jsx(GestureDetector, { gesture: composedGestures, children: _jsx(View, { ref: ref, style: [{ opacity: context.isOpen ? 0 : 1 }, style], children: children({
                    ...
                    }) }) }), isFocused && image && measurement && (_jsx(Portal, { children: _jsx(TriggerClone, { label: contentLabel, translation: translationSV, animation: animationSV, image: image, measurement: measurement, onDisplay: () => {
 ...
 */
function TriggerClone({ translation, animation, image, measurement, onDisplay, label, }) {
    const { _ } = useLingui();
    ...
    return (_jsx(Animated.View, { style: [
            ...
            animatedStyles,
                ...
                width: measurement.width,
                height: measurement.height,
            }, accessibilityLabel: label, accessibilityHint: _(msg `The subject of the context menu`), accessibilityIgnoresInvertColors: false }) }));
}
export function AuxiliaryView({ children, align = 'left' }) {
    const context = useContextMenuContext();
...
export function Outer({ children, style, align = 'left', }) {
    const t = useTheme();
    const context = useContextMenuContext();
    ...
    const onLayout = useCallback((evt) => {
        ...
        const topPosition = context.measurement.y + context.measurement.height + tokens.space.xs;
        const bottomPosition = topPosition + height;
        const safeAreaBottomLimit = frame.height -
            platform({
                   ...
                   to determine if a translation is necessary. also has the positioning */
                    _jsx(Animated.View, { onLayout: onLayout, style: [
                            ...
                            animatedContainerStyle,
                                ...
                                ], children: flattenReactChildren(children).map((child, i) => {
...
export function Item({ children, label, unstyled, style, onPress, position, ...rest }) {
    const t = useTheme();
    const context = useContextMenuContext();
    const playHaptic = useHaptics();
    ...
    const itemContext = useMemo(() => ({ disabled: Boolean(rest.disabled) }), [rest.disabled]);
    return (_jsx(Pressable, { ...rest, onLayout: handleLayout, accessibilityHint: "", accessibilityLabel: label, onFocus: onFocus, onBlur: onBlur, onPress: e => {
            close();
            onPress?.(e);
        }, onPressIn: e => {
            onPressIn();
...
export function ItemIcon({ icon: Comp }) {
    const t = useTheme();
    const { disabled } = useContextMenuItemContext();
...
export function LabelText({ children }) {
    const t = useTheme();
    return (_jsx(Text, { style: [a.font_bold, t.atoms.text_contrast_medium, { marginBottom: -8 }], children: children }));
}
