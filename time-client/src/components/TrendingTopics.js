...
import {} from '#/state/queries/trending/useTrendingTopics';
import { atoms as a, native, useTheme } from '#/alf';
import { StarterPack as StarterPackIcon } from '#/components/icons/StarterPack';
import { Link as InternalLink } from '#/components/Link';
import { Text } from '#/components/Typography';
export function TrendingTopic({ topic: raw, size, style, }) {
    const t = useTheme();
...
export function TrendingTopicSkeleton({ size = 'large', index = 0, }) {
    const t = useTheme();
    const isSmall = size === 'small';
...
export function TrendingTopicLink({ topic: raw, children, ...rest }) {
    const topic = useTopic(raw);
    return (_jsx(InternalLink, { label: topic.label, to: topic.url, PressableComponent: native(PressableScale), ...rest, children: children }));
}
