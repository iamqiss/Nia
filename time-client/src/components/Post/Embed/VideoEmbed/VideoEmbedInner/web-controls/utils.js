...
export function useVideoElement(ref) {
        ...
        const handleCanPlay = async () => {
            ...
            if (playWhenReadyRef.current) {
                try {
                    ...
                    if (!e.message?.includes(`The request is not allowed by the user agent`) &&
                        !e.message?.includes(`The play() request was interrupted by a call to pause()`)) {
                        throw e;
                    }
                }
    ...
    const changeMuted = useCallback((newMuted) => {
        if (!ref.current)
            return;
