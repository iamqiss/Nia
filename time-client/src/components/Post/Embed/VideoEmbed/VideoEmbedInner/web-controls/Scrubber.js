...
export function Scrubber({ duration, currentTime, onSeek, onSeekEnd, onSeekStart, seekLeft, seekRight, togglePlayPause, drawFocus, }) {
    const { _ } = useLingui();
    const t = useTheme();
    ...
    useEffect(() => {
        ...
        if (focused) {
            ...
            circleRef.current.addEventListener('keydown', evt => {
                // space: play/pause
    ...
    return (_jsx(View, { testID: "scrubber", style: [
            ...
            a.px_xs,
                ...
                padding: '4px 0',
                        ...
                        ] })) }), _jsx("div", { ref: circleRef, "aria-label": _(msg `Seek slider. Use the arrow keys to seek forwards and backwards, and space to play/pause`), role: "slider", "aria-valuemax": duration, "aria-valuemin": 0, "aria-valuenow": currentTime, "aria-valuetext": _(msg `${formatTime(currentTime)} of ${formatTime(duration)}`), tabIndex: 0, onFocus: onFocus, onBlur: onBlur, style: {
                        position: 'absolute',
                        height: 16,
                        width: 16,
