...
export function VideoEmbed({ embed, crop, }) {
    ...
    const contents = (_jsx("div", { ref: ref, style: { display: 'flex', flex: 1, cursor: 'default' }, onClick: evt => evt.stopPropagation(), children: _jsx(ErrorBoundary, { renderError: renderError, children: _jsx(OnlyNearScreen, { children: _jsx(VideoEmbedInnerWeb, { embed: embed, active: active, setActive: setActive, onScreen: onScreen, lastKnownTime: lastKnownTime }) }) }, key) }));
    return (_jsx(View, { style: [a.pt_xs], children: _jsx(ViewportObserver, { sendPosition: sendPosition, isAnyViewActive: currentActiveView !== null, children: cropDisabled ? (_jsx(View, { style: [a.w_full, a.overflow_hidden, { aspectRatio: max ?? 1 }], children: contents })) : (_jsx(ConstrainedImage, { fullBleed: crop === 'square', aspectRatio: constrained || 1, children: contents })) }) }));
}
 ...
 */
function ViewportObserver({ children, sendPosition, isAnyViewActive, }) {
    const ref = useRef(null);
    ...
    return (_jsxs(View, { style: [a.flex_1, a.flex_row], children: [_jsx(NearScreenContext.Provider, { value: nearScreen, children: children }), _jsx("div", { ref: ref, style: {
