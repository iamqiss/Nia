...
export function usePolicyUpdateState({ enabled, }) {
    const nux = useNux(ACTIVE_UPDATE_ID);
    const { mutate: save, variables } = useSaveNux();
    const deviceStorage = useStorage(device, [ACTIVE_UPDATE_ID]);
    const debugOverride = !!useStorage(device, ['policyUpdateDebugOverride'])[0] && IS_DEV;
    return useMemo(() => {
        /**
...
export function computeCompletedState({ nuxIsReady, nuxIsCompleted, nuxIsOptimisticallyCompleted, completedForDevice, }) {
    /**
...
export function syncCompletedState({ nuxIsReady, nuxIsCompleted, nuxIsOptimisticallyCompleted, completedForDevice, save, setCompletedForDevice, }) {
    /*
...
//# sourceMappingURL=usePolicyUpdateState.js.map