...
describe('general functionality', () => {
    ...
    test('supports nullish/falsy metadata', () => {
        ...
        logger.warn('a', null);
        expect(mockTransport).toHaveBeenCalledWith(LogLevel.Warn, undefined, 'a', {}, timestamp);
        // @ts-expect-error testing the JS case
        logger.warn('b', false);
        expect(mockTransport).toHaveBeenCalledWith(LogLevel.Warn, undefined, 'b', {}, timestamp);
        // @ts-expect-error testing the JS case
        logger.warn('c', 0);
        expect(mockTransport).toHaveBeenCalledWith(LogLevel.Warn, undefined, 'c', {}, timestamp);
        remove();
    ...
    test('add/remove transport', () => {
        ...
        expect(mockTransport).toHaveBeenNthCalledWith(1, LogLevel.Warn, undefined, 'warn', {}, timestamp);
    });
});
describe('create', () => {
    test('create', () => {
        ...
        logger.addTransport(mockTransport);
        logger.info(message, {});
        expect(mockTransport).toHaveBeenCalledWith(LogLevel.Info, Logger.Context.Default, message, {}, timestamp);
    });
});
describe('debug contexts', () => {
    test('specific', () => {
        ...
        expect(mockTransport).toHaveBeenCalledWith(LogLevel.Debug, 'specific', message, {}, timestamp);
    });
    test('namespaced', () => {
        const mockTransport = jest.fn();
        const timestamp = Date.now();
        ...
        logger.addTransport(mockTransport);
        logger.debug(message, {});
        expect(mockTransport).toHaveBeenCalledWith(LogLevel.Debug, 'namespace:foo', message, {}, timestamp);
    });
    test('ignores inactive', () => {
        ...
        logger.addTransport(mockTransport);
        logger.debug(message, {});
        expect(mockTransport).not.toHaveBeenCalledWith(LogLevel.Debug, 'namespace:bar:baz', message, {}, timestamp);
    });
});
describe('supports levels', () => {
    test('debug', () => {
        ...
        logger.addTransport(mockTransport);
        logger.debug(message);
        expect(mockTransport).toHaveBeenCalledWith(LogLevel.Debug, undefined, message, {}, timestamp);
        logger.info(message);
        expect(mockTransport).toHaveBeenCalledWith(LogLevel.Info, undefined, message, {}, timestamp);
        logger.warn(message);
        expect(mockTransport).toHaveBeenCalledWith(LogLevel.Warn, undefined, message, {}, timestamp);
        const e = new Error(message);
        logger.error(e);
        expect(mockTransport).toHaveBeenCalledWith(LogLevel.Error, undefined, e, {}, timestamp);
    });
    test('info', () => {
        const timestamp = Date.now();
        const logger = new Logger({
