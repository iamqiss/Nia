...
export function BottomBarWeb() {
    ...
    return (_jsx(Animated.View, { role: "navigation", style: [
        ...
        ], onLayout: event => footerHeight.set(event.nativeEvent.layout.height), children: hasSession ? (_jsxs(_Fragment, { children: [_jsx(NavItem, { routeName: "Home", href: "/", hasNew: hasHomeBadge && gate('remove_show_latest_button'), children: ({ isActive }) => {
                        const Icon = isActive ? HomeFilled : Home;
                        return (_jsx(Icon, { "aria-hidden": true, width: iconWidth + 1, style: [styles.ctrlIcon, t.atoms.text, styles.homeIcon] }));
                    } }), _jsx(NavItem, { routeName: "Search", href: "/search", children: ({ isActive }) => {
                        const Icon = isActive ? MagnifyingGlassFilled : MagnifyingGlass;
                        return (_jsx(Icon, { "aria-hidden": true, width: iconWidth + 2, style: [styles.ctrlIcon, t.atoms.text, styles.searchIcon] }));
                    } }), hasSession && (_jsxs(_Fragment, { children: [_jsx(NavItem, { routeName: "Messages", href: "/messages", notificationCount: unreadMessageCount.numUnread, hasNew: unreadMessageCount.hasNew, children: ({ isActive }) => {
                            ...
                            } }), _jsx(NavItem, { routeName: "Notifications", href: "/notifications", notificationCount: notificationCountStr, children: ({ isActive }) => {
...
const NavItem = ({ children, href, routeName, hasNew, notificationCount }) => {
    const { _ } = useLingui();
    ...
    const isOnDifferentProfile = currentRoute.name === 'Profile' &&
        routeName === 'Profile' &&
        currentRoute.params.name !==
            currentAccount?.handle;
    const isActive = currentRoute.name === 'Profile'
        ? isTab(currentRoute.name, routeName) &&
            currentRoute.params.name ===
                (routeName === 'Profile'
                    ...
                    : currentRoute.params.name)
        : isTab(currentRoute.name, routeName);
    return (_jsxs(Link, { href: href, style: [styles.ctrl, a.pb_lg], navigationAction: isOnDifferentProfile ? 'push' : 'navigate', "aria-role": "link", "aria-label": routeName, accessible: true, children: [children({ isActive }), notificationCount ? (_jsx(View, { style: [styles.notificationCount, styles.notificationCountWeb], "aria-label": _(msg `${plural(notificationCount, {
                    one: '# unread item',
                    other: '# unread items',
...
//# sourceMappingURL=BottomBarWeb.js.map