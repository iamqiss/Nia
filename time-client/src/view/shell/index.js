...
function DrawerLayout({ children }) {
    ...
    const renderDrawerContent = useCallback(() => _jsx(DrawerContent, {}), []);
    ...
    return (_jsx(Drawer, { renderDrawerContent: renderDrawerContent, drawerStyle: { width: Math.min(400, winDim.width * 0.8) }, configureGestureHandler: handler => {
        ...
        }, open: isDrawerOpen, onOpen: onOpenDrawer, onClose: onCloseDrawer, swipeEdgeWidth: winDim.width, swipeMinVelocity: 100, swipeMinDistance: 10, drawerType: isIOS ? 'slide' : 'front', overlayStyle: {
...
export function Shell() {
    ...
    return (_jsxs(View, { testID: "mobileShellView", style: [a.h_full, t.atoms.bg], children: [_jsx(SystemBars, { style: {
                    statusBar: t.name !== 'light' || (isIOS && fullyExpandedCount > 0)
                        ? 'light'
                        : 'dark',
                    navigationBar: t.name !== 'light' ? 'light' : 'dark',
                } }), geolocation?.isAgeBlockedGeo ? (_jsx(BlockedGeoOverlay, {})) : (_jsx(RoutesContainer, { children: _jsx(ShellInner, {}) }))] }));
}
