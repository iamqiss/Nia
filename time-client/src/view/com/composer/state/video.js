    ...
    if (action.type === 'to_error') {
        ...
        if (state.status === 'processing') {
            return {
                ...
                progress: action.jobStatus.progress !== undefined
...
export async function processVideo(asset, dispatch, agent, did, signal, _) {
    let video;
    try {
...
function getCompressErrorMessage(e, _) {
    ...
    if (e instanceof VideoTooLargeError) {
        return _(msg `The selected video is larger than 100 MB. Please try again with a smaller file.`);
    }
    ...
    if (e instanceof ServerError || e instanceof UploadLimitError) {
        ...
        switch (e.message) {
            ...
            case 'Uploading is disabled at the moment':
                return _(msg `Hold up! We’re gradually giving access to video, and you’re still waiting in line. Check back soon!`);
