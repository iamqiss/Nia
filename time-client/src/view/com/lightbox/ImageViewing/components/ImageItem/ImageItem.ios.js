...
import Animated, { runOnJS, useAnimatedProps, useAnimatedReaction, useAnimatedRef, useAnimatedStyle, useSharedValue, } from 'react-native-reanimated';
...
const ImageItem = ({ imageSrc, onTap, onZoom, onLoad, showControls, measureSafeArea, imageAspect, imageDimensions, dismissSwipePan, transforms, }) => {
    const scrollViewRef = useAnimatedRef();
    const [scaled, setScaled] = useState(false);
    const isDragging = useSharedValue(false);
    const screenSizeDelayedForJSThreadOnly = useSafeAreaFrame();
    const maxZoomScale = Math.max(MIN_SCREEN_ZOOM, imageDimensions
...
const getZoomRectAfterDoubleTap = (imageAspect, touchX, touchY, screenSize) => {
    'worklet';
    if (!imageAspect) {
        return {
            x: 0,
