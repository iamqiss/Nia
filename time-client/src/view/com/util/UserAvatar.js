...
let DefaultAvatar = ({ type, shape: overrideShape, size, }) => {
...
let UserAvatar = ({ type = 'user', shape: overrideShape, size, avatar, moderation, usePlainRNImage = false, onLoad, style, live, hideLiveBadge, noBorder, }) => {
    const t = useTheme();
    const finalShape = overrideShape ?? (type === 'user' ? 'circle' : 'square');
    const aviStyle = useMemo(() => {
        let borderRadius;
    ...
    return avatar &&
        !((moderation?.blur && isAndroid) /* android crashes with blur */) ? (_jsxs(View, { style: containerStyle, children: [usePlainRNImage ? (_jsx(Image, { accessibilityIgnoresInvertColors: true, testID: "userAvatarImage", style: aviStyle, resizeMode: "cover", source: {
                    uri: hackModifyThumbnailPath(avatar, size < 90),
                }, blurRadius: moderation?.blur ? BLUR_AMOUNT : 0, onLoad: onLoad })) : (_jsx(HighPriorityImage, { testID: "userAvatarImage", style: aviStyle, contentFit: "cover", source: {
                    uri: hackModifyThumbnailPath(avatar, size < 90),
...
export { UserAvatar };
let EditableUserAvatar = ({ type = 'user', size, avatar, onSelectNewAvatar, }) => {
    const t = useTheme();
    const { _ } = useLingui();
    ...
    const onOpenCamera = useCallback(async () => {
        ...
        onSelectNewAvatar(await compressIfNeeded(await openCamera({
            aspect: [1, 1],
        })));
    ...
    const onOpenLibrary = useCallback(async () => {
        ...
        try {
            if (isNative) {
                onSelectNewAvatar(await compressIfNeeded(await openCropper({
                    imageUri: item.path,
    ...
    return (_jsxs(_Fragment, { children: [_jsxs(Menu.Root, { children: [_jsx(Menu.Trigger, { label: _(msg `Edit avatar`), children: ({ props }) => (_jsxs(Pressable, { ...props, testID: "changeAvatarBtn", children: [avatar ? (_jsx(HighPriorityImage, { testID: "userAvatarImage", style: aviStyle, source: { uri: avatar }, accessibilityRole: "image" })) : (_jsx(DefaultAvatar, { type: type, size: size })), _jsx(View, { style: [
                                        styles.editButtonContainer,
                                        t.atoms.bg_contrast_25,
                                        a.border,
                                        t.atoms.border_contrast_low,
                                    ], children: _jsx(CameraFilledIcon, { height: 14, width: 14, style: t.atoms.text }) })] })) }), _jsxs(Menu.Outer, { showCancel: true, children: [_jsxs(Menu.Group, { children: [isNative && (_jsxs(Menu.Item, { testID: "changeAvatarCameraBtn", label: _(msg `Upload from Camera`), onPress: onOpenCamera, children: [_jsx(Menu.ItemText, { children: _jsx(Trans, { children: "Upload from Camera" }) }), _jsx(Menu.ItemIcon, { icon: CameraIcon })] })), _jsxs(Menu.Item, { testID: "changeAvatarLibraryBtn", label: _(msg `Upload from Library`), onPress: onOpenLibrary, children: [_jsx(Menu.ItemText, { children: isNative ? (_jsx(Trans, { children: "Upload from Library" })) : (_jsx(Trans, { children: "Upload from Files" })) }), _jsx(Menu.ItemIcon, { icon: LibraryIcon })] })] }), !!avatar && (_jsxs(_Fragment, { children: [_jsx(Menu.Divider, {}), _jsx(Menu.Group, { children: _jsxs(Menu.Item, { testID: "changeAvatarRemoveBtn", label: _(msg `Remove Avatar`), onPress: onRemoveAvatar, children: [_jsx(Menu.ItemText, { children: _jsx(Trans, { children: "Remove Avatar" }) }), _jsx(Menu.ItemIcon, { icon: TrashIcon })] }) })] }))] })] }), _jsx(EditImageDialog, { control: editImageDialogControl, image: rawImage, onChange: onChangeEditImage, aspectRatio: 1, circularCrop: circular })] }));
};
EditableUserAvatar = memo(EditableUserAvatar);
export { EditableUserAvatar };
let PreviewableUserAvatar = ({ moderation, profile, disableHoverCard, disableNavigation, onBeforePress, live, ...props }) => {
    const { _ } = useLingui();
    ...
    const onOpenLiveStatus = useCallback(() => {
        playHaptic('Light');
        logger.metric('live:card:open', { subject: profile.did, from: 'post' }, { statsig: true });
        liveControl.open();
    }, [liveControl, playHaptic, profile.did]);
    const avatarEl = (_jsx(UserAvatar, { avatar: profile.avatar, moderation: moderation, type: profile.associated?.labeler ? 'labeler' : 'user', live: status.isActive || live, ...props }));
    const linkStyle = props.type !== 'algo' && props.type !== 'list'
        ? a.rounded_full
        : { borderRadius: props.size > 32 ? 8 : 3 };
    return (_jsx(ProfileHoverCard, { did: profile.did, disable: disableHoverCard, children: disableNavigation ? (avatarEl) : status.isActive && (isNative || isTouchDevice) ? (_jsxs(_Fragment, { children: [_jsx(Button, { label: _(msg `${sanitizeDisplayName(profile.displayName || sanitizeHandle(profile.handle))}'s avatar`), accessibilityHint: _(msg `Opens live status dialog`), onPress: onOpenLiveStatus, children: avatarEl }), _jsx(LiveStatusDialog, { control: liveControl, profile: profile, status: status, embed: status.embed })] })) : (_jsx(Link, { label: _(msg `${sanitizeDisplayName(profile.displayName || sanitizeHandle(profile.handle))}'s avatar`), accessibilityHint: _(msg `Opens this profile`), to: makeProfileLink({
                did: profile.did,
