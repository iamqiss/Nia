...
let PostFeed = ({ feed, feedParams, ignoreFilterFor, style, enabled, pollInterval, disablePoll, scrollElRef, onScrolledDownChange, onHasNew, renderEmptyState, renderEndOfFeed, testID, headerOffset = 0, progressViewOffset, desktopFixedHeightOffset, ListHeaderComponent, extraData, savedFeedConfig, initialNumToRender: initialNumToRenderOverride, isVideoFeed = false, }) => {
    const { _ } = useLingui();
    ...
    const [hasPressedShowLessUris, setHasPressedShowLessUris] = useState(() => new Set());
    const onPressShowLess = useCallback((interaction) => {
        if (interaction.item) {
    ...
    const { data, isFetching, isFetched, isError, error, refetch, hasNextPage, isFetchingNextPage, fetchNextPage, } = usePostFeedQuery(feed, feedParams, opts);
    const lastFetchedAt = data?.pages[0].fetchedAt;
    if (lastFetchedAt) {
        lastFetchRef.current = lastFetchedAt;
    }
    const isEmpty = useMemo(() => !isFetching && !data?.pages?.some(page => page.slices.length), [isFetching, data]);
                        ...
                        for (const slice of page.slices) {
                            ...
                            if (slice.isFallbackMarker) {
                                ...
                                arr.push(sliceItem({
                                    ...
                                    showReplyTo: slice.items[beforeLast].parentAuthor?.did !==
    ...
    const renderItem = useCallback(({ item: row, index: rowIndex }) => {
        if (row.type === 'empty') {
            ...
            return (_jsx(PostFeedErrorMessage, { feedDesc: feed, error: error ?? undefined, onPressTryAgain: onPressTryAgain, savedFeedConfig: savedFeedConfig }));
        }
        else if (row.type === 'loadMoreError') {
            return (_jsx(LoadMoreRetryBtn, { label: _(msg `There was an issue fetching posts. Tap here to try again.`), onPress: onPressRetryLoadMore }));
        }
    ...
    return (_jsx(View, { testID: testID, style: style, children: _jsx(List, { testID: testID ? `${testID}-flatlist` : undefined, ref: scrollElRef, data: feedItems, keyExtractor: item => item.key, renderItem: renderItem, ListFooterComponent: FeedFooter, ListHeaderComponent: ListHeaderComponent, refreshing: isPTRing, onRefresh: onRefresh, headerOffset: headerOffset, progressViewOffset: progressViewOffset, contentContainerStyle: {
                minHeight: Dimensions.get('window').height * 1.5,
            }, onScrolledDownChange: onScrolledDownChange, onEndReached: onEndReached, onEndReachedThreshold: 2, removeClippedSubviews: true, extraData: extraData, desktopFixedHeight: desktopFixedHeightOffset ? desktopFixedHeightOffset : true, initialNumToRender: initialNumToRenderOverride ?? initialNumToRender, windowSize: 9, maxToRenderPerBatch: isIOS ? 5 : 1, updateCellsBatchingPeriod: 40, onItemSeen: onItemSeen }) }));
};
...
//# sourceMappingURL=PostFeed.js.map