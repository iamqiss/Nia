...
export function PostLikedBy({ uri }) {
    const { _ } = useLingui();
    const initialNumToRender = useInitialNumToRender();
    const [isPTRing, setIsPTRing] = useState(false);
    const { data: resolvedUri, error: resolveError, isLoading: isLoadingUri, } = useResolveUriQuery(uri);
    const { data, isLoading: isLoadingLikes, isFetchingNextPage, hasNextPage, fetchNextPage, error, refetch, } = useLikedByQuery(resolvedUri?.uri);
    ...
    if (likes.length < 1) {
        return (_jsx(ListMaybePlaceholder, { isLoading: isLoadingUri || isLoadingLikes, isError: isError, emptyType: "results", emptyTitle: _(msg `No likes yet`), emptyMessage: _(msg `Nobody has liked this yet. Maybe you should be the first!`), errorMessage: cleanError(resolveError || error), sideBorders: false, topBorder: false }));
    }
