...
export function ModerationBlockedAccounts({}) {
    ...
    const { data, isFetching, isError, error, refetch, hasNextPage, fetchNextPage, isFetchingNextPage, } = useMyBlockedAccountsQuery();
    ...
    const renderItem = ({ item, index, }) => {
        if (!moderationOpts)
            return null;
        return (_jsx(View, { style: [a.py_md, a.px_xl, a.border_t, t.atoms.border_contrast_low], children: _jsx(ProfileCard.Default, { testID: `blockedAccount-${index}`, profile: item, moderationOpts: moderationOpts }) }, item.did));
    };
    return (_jsx(Layout.Screen, { testID: "blockedAccountsScreen", children: _jsxs(Layout.Center, { children: [_jsxs(Layout.Header.Outer, { children: [_jsx(Layout.Header.BackButton, {}), _jsx(Layout.Header.Content, { children: _jsx(Layout.Header.TitleText, { children: _jsx(Trans, { children: "Blocked Accounts" }) }) }), _jsx(Layout.Header.Slot, {})] }), isEmpty ? (_jsxs(View, { children: [_jsx(Info, { style: [a.border_b] }), isError ? (_jsx(ErrorScreen, { title: "Oops!", message: cleanError(error), onPressTryAgain: refetch })) : (_jsx(Empty, {}))] })) : (_jsx(List, { data: profiles, keyExtractor: (item) => item.did, refreshing: isPTRing, onRefresh: onRefresh, onEndReached: onEndReached, renderItem: renderItem, initialNumToRender: 15, 
                    // FIXME(dan)
...
//# sourceMappingURL=ModerationBlockedAccounts.js.map