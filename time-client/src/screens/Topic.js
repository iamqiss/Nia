...
export default function TopicScreen({ route, }) {
    ...
    return (_jsx(Layout.Screen, { children: _jsx(Pager, { onPageSelected: onPageSelected, renderTabBar: props => (_jsxs(Layout.Center, { style: [a.z_10, web([a.sticky, { top: 0 }])], children: [_jsxs(Layout.Header.Outer, { noBottomBorder: true, children: [_jsx(Layout.Header.BackButton, {}), _jsx(Layout.Header.Content, { children: _jsx(Layout.Header.TitleText, { children: headerTitle }) }), _jsx(Layout.Header.Slot, { children: _jsx(Button, { label: _(msg `Share`), size: "small", variant: "ghost", color: "primary", shape: "round", onPress: onShare, hitSlop: HITSLOP_10, style: [{ right: -3 }], children: _jsx(ButtonIcon, { icon: Share, size: "md" }) }) })] }), _jsx(TabBar, { items: sections.map(section => section.title), ...props })] })), initialPage: 0, children: sections.map((section, i) => (_jsx(View, { children: section.component }, i))) }) }));
}
function TopicScreenTab({ topic, sort, active, }) {
    const { _ } = useLingui();
    const initialNumToRender = useInitialNumToRender();
    const [isPTR, setIsPTR] = React.useState(false);
    const { data, isFetched, isFetchingNextPage, isLoading, isError, error, refetch, fetchNextPage, hasNextPage, } = useSearchPostsQuery({
    ...
    return (_jsx(_Fragment, { children: posts.length < 1 ? (_jsx(ListMaybePlaceholder, { isLoading: isLoading || !isFetched, isError: isError, onRetry: refetch, emptyType: "results", emptyMessage: _(msg `We couldn't find any results for that topic.`) })) : (_jsx(List, { data: posts, renderItem: renderItem, keyExtractor: keyExtractor, refreshing: isPTR, onRefresh: onRefresh, onEndReached: onEndReached, onEndReachedThreshold: 4, 
