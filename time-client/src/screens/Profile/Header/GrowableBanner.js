...
export function GrowableBanner({ backButton, children, }) {
    const pagerContext = usePagerHeaderContext();
    // plain non-growable mode for Android/Web
    if (!pagerContext || !isIOS) {
        return (_jsxs(View, { style: [a.w_full, a.h_full], children: [children, backButton] }));
    }
    const { scrollY } = pagerContext;
    return (_jsx(GrowableBannerInner, { scrollY: scrollY, backButton: backButton, children: children }));
}
function GrowableBannerInner({ scrollY, backButton, children, }) {
    ...
    return (_jsxs(_Fragment, { children: [_jsxs(Animated.View, { style: [
                    ...
                    animatedStyle,
                ], children: [children, _jsx(AnimatedBlurView, { style: [a.absolute, a.inset_0], tint: "dark", animatedProps: animatedBlurViewProps })] }), _jsx(View, { style: [
                    a.absolute,
                    a.inset_0,
...
function useShouldAnimateSpinner({ isFetching, scrollY, }) {
    ...
    useAnimatedReaction(() => scrollY.get() < -5, (value, prevValue) => {
        if (value !== prevValue) {
...
//# sourceMappingURL=GrowableBanner.js.map