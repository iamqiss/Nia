...
export function Header({ rkey, list, preferences, }) {
    const { _ } = useLingui();
    ...
    const descriptionRT = useMemo(() => list.description
        ...
        : undefined, [list]);
    return (_jsxs(_Fragment, { children: [_jsxs(ProfileSubpageHeader, { href: makeListLink(list.creator.handle || list.creator.did || '', rkey), title: list.name, avatar: list.avatar, isOwner: list.creator.did === currentAccount?.did, creator: list.creator, purpose: list.purpose, avatarType: "list", children: [isCurateList ? (_jsxs(Button, { testID: isPinned ? 'unpinBtn' : 'pinBtn', color: isPinned ? 'secondary' : 'primary_subtle', label: isPinned ? _(msg `Unpin`) : _(msg `Pin to home`), onPress: onTogglePinned, disabled: isPending, size: "small", style: [a.rounded_full], children: [!isPinned && _jsx(ButtonIcon, { icon: isPending ? Loader : PinIcon }), _jsx(ButtonText, { children: isPinned ? _jsx(Trans, { children: "Unpin" }) : _jsx(Trans, { children: "Pin to home" }) })] })) : isModList ? (isBlocking ? (_jsxs(Button, { testID: "unblockBtn", color: "secondary", label: _(msg `Unblock`), onPress: onUnsubscribeBlock, size: "small", style: [a.rounded_full], disabled: isBlockPending, children: [isBlockPending && _jsx(ButtonIcon, { icon: Loader }), _jsx(ButtonText, { children: _jsx(Trans, { children: "Unblock" }) })] })) : isMuting ? (_jsxs(Button, { testID: "unmuteBtn", color: "secondary", label: _(msg `Unmute`), onPress: onUnsubscribeMute, size: "small", style: [a.rounded_full], disabled: isMutePending, children: [isMutePending && _jsx(ButtonIcon, { icon: Loader }), _jsx(ButtonText, { children: _jsx(Trans, { children: "Unmute" }) })] })) : (_jsx(SubscribeMenu, { list: list }))) : null, _jsx(MoreOptionsMenu, { list: list })] }), descriptionRT ? (_jsx(View, { style: [a.px_lg, a.pt_sm, a.pb_sm, a.gap_md], children: _jsx(RichText, { value: descriptionRT, style: [a.text_md, a.leading_snug] }) })) : null] }));
}
