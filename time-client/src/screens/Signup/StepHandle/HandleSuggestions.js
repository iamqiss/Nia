...
export function HandleSuggestions({ suggestions, onSelect, }) {
    const t = useTheme();
    const { _ } = useLingui();
    return (_jsx(Animated.View, { entering: native(FadeInDown.easing(Easing.out(Easing.exp))), exiting: native(FadeOut), style: [
            ...
            a.w_full,
            a.zoom_fade_in,
        ], children: suggestions.map((suggestion, index) => (_jsxs(Button, { label: _(msg({
                message: `Select ${suggestion.handle}`,
                comment: `Accessibility label for a username suggestion in the account creation flow`,
            })), onPress: () => onSelect(suggestion), hoverStyle: [t.atoms.bg_contrast_25], style: [
                a.w_full,
                a.flex_row,
                a.align_center,
                a.justify_between,
...
//# sourceMappingURL=HandleSuggestions.js.map