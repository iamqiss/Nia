...
export function Onboarding() {
    ...
    const showSuggestedAccounts = ENV !== 'e2e' && gate('onboarding_suggested_accounts');
    ...
    return (_jsx(Portal, { children: _jsx(OnboardingControls.Provider, { children: _jsx(OnboardingHeaderSlot.Provider, { children: _jsx(Context.Provider, { value: useMemo(() => ({ state, dispatch, interestsDisplayNames }), [state, dispatch, interestsDisplayNames]), children: _jsxs(Layout, { children: [state.activeStep === 'profile' && _jsx(StepProfile, {}), state.activeStep === 'interests' && _jsx(StepInterests, {}), state.activeStep === 'suggested-accounts' && (_jsx(StepSuggestedAccounts, {})), state.activeStep === 'finished' && _jsx(StepFinished, {})] }) }) }) }) }));
}
