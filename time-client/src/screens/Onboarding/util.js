...
export async function bulkWriteFollows(agent, dids) {
    const session = agent.session;
    if (!session) {
        throw new Error(`bulkWriteFollows failed: no session`);
    }
    const followRecords = dids.map(did => {
        return {
...
async function whenFollowsIndexed(agent, actor, fn) {
    await until(5, // 5 tries
    1e3, // 1s delay between tries
    fn, () => agent.app.bsky.graph.getFollows({
        actor,
        limit: 1,
    }));
}
