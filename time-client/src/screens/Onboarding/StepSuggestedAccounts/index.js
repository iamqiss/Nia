...
import { Context, popularInterests, useInterestsDisplayNames, } from '#/screens/Onboarding/state';
...
export function StepSuggestedAccounts() {
    ...
    const { data: suggestedUsers, isLoading, error, isRefetching, refetch, } = useSuggestedUsers({
    ...
    const followableDids = suggestedUsers?.actors
        .filter(user => user.did !== currentAccount?.did &&
    ...
    const { mutate: followAll, isPending: isFollowingAll } = useMutation({
        ...
        onError: () => {
            toast.show(_(msg `Failed to follow all suggested accounts, please try again`), { type: 'error' });
        },
    });
    const canFollowAll = followableDids.length > 0 && !isFollowingAll;
    return (_jsxs(View, { style: [a.align_start], testID: "onboardingInterests", children: [_jsx(Text, { style: [a.font_heavy, a.text_3xl], children: _jsx(Trans, { comment: "Accounts suggested to the user for them to follow", children: "Suggested for you" }) }), _jsxs(View, { style: [
                    a.overflow_hidden,
                    a.mt_lg,
                    ...
                    a.justify_start,
                            ...
                            a.justify_center,
                            { minHeight: 400 },
                        ], children: _jsx(Loader, { size: "xl" }) })) : isError ? (_jsx(View, { style: [a.flex_1, a.px_xl, a.pt_5xl], children: _jsx(Admonition, { type: "error", children: _jsx(Trans, { children: "An error occurred while fetching suggested accounts." }) }) })) : (_jsx(View, { style: [
                            a.flex_1,
                            a.mt_md,
                            a.border_y,
                            t.atoms.border_contrast_low,
                            isWeb && [a.border_x, a.rounded_sm, a.overflow_hidden],
                        ], children: suggestedUsers?.actors.map((user, index) => (_jsx(SuggestedProfileCard, { profile: user, moderationOpts: moderationOpts, position: index }, user.did))) }))] }), _jsx(OnboardingControls.Portal, { children: isError ? (_jsxs(View, { style: [a.gap_md, gtMobile ? a.flex_row : a.flex_col], children: [_jsxs(Button, { disabled: isRefetching, color: "secondary", size: "large", label: _(msg `Retry`), onPress: () => refetch(), children: [_jsx(ButtonText, { children: _jsx(Trans, { children: "Retry" }) }), _jsx(ButtonIcon, { icon: ArrowRotateCounterClockwiseIcon })] }), _jsx(Button, { color: "secondary", size: "large", label: _(msg `Skip this flow`), onPress: skipOnboarding, children: _jsx(ButtonText, { children: _jsx(Trans, { children: "Skip" }) }) })] })) : (_jsxs(View, { style: [a.gap_md, gtMobile ? a.flex_row : a.flex_col], children: [_jsxs(Button, { disabled: !canFollowAll, color: "secondary", size: "large", label: _(msg `Follow all accounts`), onPress: () => followAll(), children: [_jsx(ButtonText, { children: _jsx(Trans, { children: "Follow all" }) }), _jsx(ButtonIcon, { icon: isFollowingAll ? Loader : PlusIcon })] }), _jsx(Button, { disabled: isFollowingAll, color: "primary", size: "large", label: _(msg `Continue to next step`), onPress: () => dispatch({ type: 'next' }), children: _jsx(ButtonText, { children: _jsx(Trans, { children: "Continue" }) }) })] })) })] }));
}
function TabBar({ selectedInterest, onSelectInterest, selectedInterests, hideDefaultTab, defaultTabLabel, }) {
    const { _ } = useLingui();
    const interestsDisplayNames = useInterestsDisplayNames();
    const interests = Object.keys(interestsDisplayNames)
        .sort(boostInterests(popularInterests))
        .sort(boostInterests(selectedInterests));
    return (_jsx(InterestTabs, { interests: hideDefaultTab ? interests : ['all', ...interests], selectedInterest: selectedInterest || (hideDefaultTab ? interests[0] : 'all'), onSelectTab: tab => {
            logger.metric('onboarding:suggestedAccounts:tabPressed', { tab: tab }, { statsig: true });
...
function SuggestedProfileCard({ profile, moderationOpts, position, }) {
    const t = useTheme();
