...
export function ThreadItemAnchorFollowButton({ did }) {
...
function PostThreadFollowBtnLoaded({ profile: profileUnshadowed, }) {
    ...
    if (!showFollowBtn)
        return null;
    return (_jsxs(Button, { testID: "followBtn", label: _(msg `Follow ${profile.handle}`), onPress: onPress, size: "small", variant: "solid", color: isFollowing ? 'secondary' : 'secondary_inverted', style: [a.rounded_full], children: [gtMobile && (_jsx(ButtonIcon, { icon: isFollowing ? Check : Plus, position: "left", size: "sm" })), _jsx(ButtonText, { children: !isFollowing ? (isFollowedBy ? (_jsx(Trans, { children: "Follow back" })) : (_jsx(Trans, { children: "Follow" }))) : (_jsx(Trans, { children: "Following" })) })] }));
}
//# sourceMappingURL=ThreadItemAnchorFollowButton.js.map