...
import { usePostThread } from '#/state/queries/usePostThread';
...
export function PostThread({ uri }) {
    ...
    const optimisticOnPostReply = useCallback((payload) => {
        if (payload) {
    ...
    const isTombstoneView = useMemo(() => {
        if (slices.length > 1)
            return false;
        return slices.every(s => s.type === 'threadPostBlocked' || s.type === 'threadPostNotFound');
    }, [slices]);
    const renderItem = useCallback(({ item, index }) => {
        if (item.type === 'threadPost') {
            if (item.depth < 0) {
                return (_jsx(ThreadItemPost, { item: item, threadgateRecord: thread.data.threadgate?.record ?? undefined, overrides: {
                        topBorder: index === 0,
                    }, onPostSuccess: optimisticOnPostReply }));
            }
                ...
                _jsxs(View, { collapsable: false, children: [_jsx(View
                         ...
                         */
                        , { ref: anchorRef, onLayout: () => setDeferParents(false) }, item.uri + thread.state.view + thread.state.sort), _jsx(ThreadItemAnchor, { item: item, threadgateRecord: thread.data.threadgate?.record ?? undefined, onPostSuccess: optimisticOnPostReply, postSource: anchorPostSource }), _jsx(LoggedOutCTA, { style: a.px_lg, gateName: "cta_above_post_replies" })] }));
            }
            else {
                if (thread.state.view === 'tree') {
                    return (_jsx(ThreadItemTreePost, { item: item, threadgateRecord: thread.data.threadgate?.record ?? undefined, overrides: {
...
function MobileComposePrompt({ onPressReply }) {
