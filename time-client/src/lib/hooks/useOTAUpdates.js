...
async function setExtraParams() {
    await setExtraParamAsync(isIOS ? 'ios-build-number' : 'android-build-number', 
    // Hilariously, `buildVersion` is not actually a string on Android even though the TS type says it is.
    // This just ensures it gets passed as a string
    `${nativeBuildVersion}`);
    await setExtraParamAsync('channel', IS_TESTFLIGHT ? 'testflight' : 'production');
}
async function setExtraParamsPullRequest(channel) {
    await setExtraParamAsync(isIOS ? 'ios-build-number' : 'android-build-number', 
    // Hilariously, `buildVersion` is not actually a string on Android even though the TS type says it is.
    // This just ensures it gets passed as a string
    `${nativeBuildVersion}`);
...
export function useApplyPullRequestOTAUpdate() {
    ...
    const isCurrentlyRunningPullRequestDeployment = currentChannel?.startsWith('pull-request');
    const tryApplyUpdate = async (channel) => {
        setPending(true);
        await setExtraParamsPullRequest(channel);
        const res = await checkForUpdateAsync();
        if (res.isAvailable) {
            Alert.alert('Deployment Available', `A deployment of ${channel} is availalble. Applying this deployment may result in a bricked installation, in which case you will need to reinstall the app and may lose local data. Are you sure you want to proceed?`, [
                {
                    text: 'No',
                    style: 'cancel',
                },
            ...
            Alert.alert('No Deployment Available', `No new deployments of ${channel} are currently available for your current native build.`);
        }
        setPending(false);
    };
...
//# sourceMappingURL=useOTAUpdates.js.map