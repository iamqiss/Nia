 ...
 * accounts.
 */
let storedAccountSwitchPayload;
/**
...
export function useNotificationsHandler() {
    ...
    useEffect(() => {
        ...
        Notifications.setNotificationChannelGroupAsync(CHAT_GROUP, {
            name: _(msg `Chat`),
            description: _(msg `You can choose whether chat notifications have sound in the chat settings within the app`),
        });
    ...
    useEffect(() => {
        ...
        const responseReceivedListener = Notifications.addNotificationResponseReceivedListener(e => {
            ...
            if (payload) {
                notyLogger.debug('User pressed a notification, opening notifications tab', {});
                notyLogger.metric('notifications:openApp', { reason: payload.reason, causedBoot: false }, { statsig: false });
                invalidateCachedUnreadPage();
        ...
        if (storedAccountSwitchPayload?.reason === 'chat-message' &&
...
export function getNotificationPayload(e) {
    ...
    const payload = (isIOS ? e.request.trigger.payload : e.request.content.data);
