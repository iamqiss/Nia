...
import {} from './types';
export async function compressIfNeeded(img, maxSize = POST_IMG_MAX.size) {
    if (img.size < maxSize) {
        return img;
    }
...
export async function downloadAndResize(opts) {
    let appendExt = 'jpeg';
    try {
...
export async function shareImageModal({ uri }) {
...
export async function saveImageToMediaLibrary({ uri }) {
    ...
    try {
        if (isAndroid) {
            ...
            if (album) {
                ...
                await MediaLibrary.createAlbumAsync(ALBUM_NAME, undefined, undefined, imagePath);
            }
        }
        else {
...
async function doResize(localUri, opts) {
...
async function moveToPermanentPath(path, ext) {
    /*
...
export async function safeDeleteAsync(path) {
...
function normalizePath(str, allPlatforms = false) {
...
export async function saveBytesToDisk(filename, bytes, type) {
    const encoded = Buffer.from(bytes).toString('base64');
    return await saveToDevice(filename, encoded, type);
}
export async function saveToDevice(filename, encoded, type) {
    try {
        if (isIOS) {
...
async function withTempFile(filename, encoded, cb) {
    ...
    const tmpDirUri = joinPath(cacheDirectory, String(uuid.v4()));
...
async function downloadImage(uri, path, timeout) {
