...
import {} from '#/view/com/composer/state/composer';
import { createGIFDescription } from '../gif-alt-text';
import { uploadBlob } from './upload-blob';
export { uploadBlob };
export async function post(agent, queryClient, opts) {
...
async function resolveRT(agent, richtext) {
...
async function resolveReply(agent, replyTo) {
...
async function resolveEmbed(agent, queryClient, draft, onStateChange) {
...
async function resolveMedia(agent, queryClient, embedDraft, onStateChange) {
    if (embedDraft.media?.type === 'images') {
        ...
        const images = await Promise.all(imagesDraft.map(async (image, i) => {
    ...
    if (embedDraft.media?.type === 'video' &&
        embedDraft.media.video.status === 'done') {
        const videoDraft = embedDraft.media.video;
        const captions = await Promise.all(videoDraft.captions
        ...
        if (!aspectRatio) {
            logger.error(`Invalid aspect ratio - got { width: ${videoDraft.asset.width}, height: ${videoDraft.asset.height} }`);
        }
        return {
...
async function resolveRecord(agent, queryClient, uri) {
...
async function computeCid(record) {
