...
export class FeedViewPostsSlice {
    ...
    constructor(feedPost) {
        ...
        this._reactKey = `slice-${post.uri}-${feedPost.reason && 'indexedAt' in feedPost.reason
        ...
        const rootIsView = AppBskyFeedDefs.isPostView(root) ||
         ...
         * it does.
         */
        const grandparent = rootIsView && parent.record.reply?.parent.uri === root.uri
            ? root
            : undefined;
...
export class FeedTuner {
    ...
    seenKeys = new Set();
    seenUris = new Set();
    seenRootUris = new Set();
    constructor(tunerFns) {
    ...
    static removeReplies(tuner, slices, _dryRun) {
    ...
    static removeReposts(tuner, slices, _dryRun) {
        for (let i = 0; i < slices.length; i++) {
    ...
    static removeQuotePosts(tuner, slices, _dryRun) {
        for (let i = 0; i < slices.length; i++) {
    ...
    static removeOrphans(tuner, slices, _dryRun) {
        for (let i = 0; i < slices.length; i++) {
    ...
    static removeMutedThreads(tuner, slices, _dryRun) {
        for (let i = 0; i < slices.length; i++) {
    ...
    static dedupThreads(tuner, slices, dryRun) {
        for (let i = 0; i < slices.length; i++) {
     ...
     */
    static preferredLangOnly(preferredLangsCode2) {
        return (tuner, slices, _dryRun) => {
...
//# sourceMappingURL=feed-manip.js.map