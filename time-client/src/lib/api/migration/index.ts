...
/**
 * Enhanced post function with gRPC/REST migration support
 */
export async function post(
  ...
  const migrationService = ApiMigrationService.getInstance();
  const featureFlags = GrpcFeatureFlagManager.getInstance();
  
  // Check if we should use gRPC for post creation
  if (featureFlags.isGrpcEnabledForOperation('createNote')) {
    try {
      return await postWithGrpc(migrationService, agent, queryClient, opts);
    } catch (error) {
      logger.warn('gRPC post creation failed, falling back to REST:', error);
      // Fall back to REST implementation
    }
  }
  
  // Use original REST implementation
  return postWithRest(agent, queryClient, opts);
}

/**
 * Create post using gRPC
 */
async function postWithGrpc(
  ...
  // Process the first post in the thread
  const draft = thread.posts[0];
  if (!draft) {
  ...
  const postData = {
    ...
    media: draft.embed.media ? [draft.embed.media] : [],
    labels: draft.labels,
    threadgate: thread.threadgate,
    postgate: thread.postgate,
  };

  const result = await migrationService.createPost(agent, queryClient, postData);
  
  opts.onStateChange?.(t`Post created successfully`);
  return result;
}

/**
 * Create post using REST (original implementation)
 */
