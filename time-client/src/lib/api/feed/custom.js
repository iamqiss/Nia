...
// -prf
async function loggedOutFetch({ feed, limit, cursor, }) {
    ...
    let res = await fetch(`https://api.bsky.app/xrpc/app.bsky.feed.getFeed?feed=${feed}${cursor ? `&cursor=${cursor}` : ''}&limit=${limit}&lang=${contentLangs}`, {
        method: 'GET',
        headers: { 'Accept-Language': contentLangs, ...labelersHeader },
    });
    let data = res.ok
        ? jsonStringToLex(await res.text())
        : null;
    if (data?.feed?.length) {
        return {
            success: true,
    ...
    res = await fetch(`https://api.bsky.app/xrpc/app.bsky.feed.getFeed?feed=${feed}${cursor ? `&cursor=${cursor}` : ''}&limit=${limit}`, { method: 'GET', headers: { 'Accept-Language': '', ...labelersHeader } });
    data = res.ok
        ? jsonStringToLex(await res.text())
        : null;
    if (data?.feed?.length) {
