 ...
 */
async function _registerPushToken({ agent, currentAccount, token, extra = {}, }) {
    try {
        const payload = {
...
export function useRegisterPushToken() {
    ...
    return useCallback(({ token, isAgeRestricted, }) => {
        if (!currentAccount)
            return;
...
export function useGetAndRegisterPushToken() {
    ...
    return useCallback(async ({ isAgeRestricted: isAgeRestrictedOverride, } = {}) => {
        if (!isNative || IS_DEV)
            return;
        /**
...
export async function decrementBadgeCount(by) {
    if (!isNative)
        return;
