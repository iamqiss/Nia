import { validateServiceHandle } from '#/lib/strings/handles';
describe('handle validation', () => {
    ...
    it.each(invalid)(`should be invalid: %s.%s due to %s`, (handle, service, expectedError) => {
        const result = validateServiceHandle(handle, service);
        expect(result.overall).toEqual(false);
        expect(result[expectedError]).toEqual(false);
    });
});
//# sourceMappingURL=handles.test.js.map