...
import { createStarterPackGooglePlayUri, createStarterPackLinkFromAndroidReferrer, parseStarterPackUri, } from '#/lib/strings/starter-pack';
...
import { makeRecordUri, toNiceDomain, toShareUrl, toShortUrl, } from '../../src/lib/strings/url-helpers';
...
describe('parseEmbedPlayerFromUrl', () => {
    ...
    const outputs = [
        ...
        undefined,
        {
            ...
            source: 'appleMusic',
            playerUri: 'https://embed.music.apple.com/us/playlist/playlistName/playlistId',
        },
        {
        ...
        },
        {
            ...
            source: 'appleMusic',
            playerUri: 'https://embed.music.apple.com/us/album/albumName/albumId?i=songId',
        },
        {
            type: 'vimeo_video',
            source: 'vimeo',
...
describe('createStarterPackLinkFromAndroidReferrer', () => {
    ...
    it('returns a link when input contains utm_source and utm_content', () => {
        expect(createStarterPackLinkFromAndroidReferrer('utm_source=bluesky&utm_content=starterpack_haileyok.com_rkey')).toEqual(validOutput);
        expect(createStarterPackLinkFromAndroidReferrer('utm_source=bluesky&utm_content=starterpack_test-lover-9000.com_rkey')).toEqual('at://test-lover-9000.com/app.bsky.graph.starterpack/rkey');
    });
    it('returns a link when input contains utm_source and utm_content in different order', () => {
        expect(createStarterPackLinkFromAndroidReferrer('utm_content=starterpack_haileyok.com_rkey&utm_source=bluesky')).toEqual(validOutput);
    });
    it('returns a link when input contains other parameters as well', () => {
        expect(createStarterPackLinkFromAndroidReferrer('utm_source=bluesky&utm_medium=starterpack&utm_content=starterpack_haileyok.com_rkey')).toEqual(validOutput);
    });
    it('returns null when utm_source is not present', () => {
        expect(createStarterPackLinkFromAndroidReferrer('utm_content=starterpack_haileyok.com_rkey')).toEqual(null);
    });
    it('returns null when utm_content is not present', () => {
        expect(createStarterPackLinkFromAndroidReferrer('utm_source=bluesky')).toEqual(null);
    });
    it('returns null when utm_content is malformed', () => {
        expect(createStarterPackLinkFromAndroidReferrer('utm_content=starterpack_haileyok.com')).toEqual(null);
        expect(createStarterPackLinkFromAndroidReferrer('utm_content=starterpack')).toEqual(null);
        expect(createStarterPackLinkFromAndroidReferrer('utm_content=starterpack_haileyok.com_rkey_more')).toEqual(null);
        expect(createStarterPackLinkFromAndroidReferrer('utm_content=notastarterpack_haileyok.com_rkey')).toEqual(null);
    });
});
...
describe('createStarterPackGooglePlayUri', () => {
    const base = 'https://play.google.com/store/apps/details?id=xyz.blueskyweb.app&referrer=utm_source%3Dbluesky%26utm_medium%3Dstarterpack%26utm_content%3Dstarterpack_';
    it('returns valid google play uri when input is valid', () => {
        expect(createStarterPackGooglePlayUri('name', 'rkey')).toEqual(`${base}name_rkey`);
    });
...
describe('tenorUrlToBskyGifUrl', () => {
    ...
    it.each(inputs)('returns url with t.gifs.bsky.app as hostname for input url', input => {
