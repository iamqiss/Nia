...
import { downloadAndResize, getResizedDimensions, } from '../../src/lib/media/manip';
...
describe('downloadAndResize', () => {
    ...
    beforeEach(() => {
        const mockedCreateResizedImage = manipulateAsync;
    ...
    it('should return resized image for valid URI and options', async () => {
        const mockedFetch = createDownloadResumable;
        ...
        expect(createDownloadResumable).toHaveBeenCalledWith(opts.uri, expect.anything(), {
            cache: true,
        });
        // First time it gets called is to get dimensions
        expect(manipulateAsync).toHaveBeenCalledWith(expect.any(String), [], {});
        expect(manipulateAsync).toHaveBeenCalledWith(expect.any(String), [{ resize: { height: opts.height, width: opts.width } }], { format: SaveFormat.JPEG, compress: 1.0 });
