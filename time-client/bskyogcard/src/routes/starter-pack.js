...
export default function (ctx, app) {
    return app.get('/start/:actor/:rkey', originVerifyMiddleware(ctx), handler(async (req, res) => {
        const { actor, rkey } = req.params;
        const uri = AtUri.make(actor, 'app.bsky.graph.starterpack', rkey);
        let starterPack;
        try {
            const result = await ctx.appviewAgent.api.app.bsky.graph.getStarterPack({ starterPack: uri.toString() });
        ...
        const imageEntries = await Promise.all([starterPack.creator]
        ...
        const images = new Map(imageEntries.filter(([_, image]) => image !== null).slice(0, 7));
        const svg = await satori(_jsx(StarterPack, { starterPack: starterPack, images: images }), {
            fonts: ctx.fonts,
